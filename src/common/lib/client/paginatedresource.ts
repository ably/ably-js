...
class PaginatedResource {
  ...
  async handlePage<T>(result: ResourceResult<T>): Promise<PaginatedResult<T>> {
    if (result._err && returnErrOnly(result._err, result._body, this.useHttpPaginatedResponse)) {
      Logger.logAction(
        Logger.LOG_ERROR,
        'PaginatedResource.handlePage()',
        'Unexpected error getting resource: err = ' + Utils.inspectError(result._err)
      );
      throw result._err;
    }
    ...
    try {
      items = await this.bodyHandler(result._body, result._headers || {}, result._unpacked);
    } catch (e) {
    ...
    if (result._headers && (linkHeader = result._headers['Link'] || result._headers['link'])) {
