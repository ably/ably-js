...
const webSocketTransportInitializerFactory = (
  ...
  class WebSocketTransport extends superclass {
    ...
    send(message: IProtocolMessage) {
      ...
      try {
        wsConnection.send(protocolMessageClass.serialize(message, this.params.format));
      } catch (e) {
    ...
    onWsData(data: string) {
      ...
      try {
        this.onProtocolMessage(protocolMessageClass.deserialize(data, this.format));
      } catch (e) {
        Logger.logAction(
          Logger.LOG_ERROR,
          'WebSocketTransport.onWsData()',
  ...
  return function initialiseTransport(connectionManager: any): typeof WebSocketTransport {
...
export default webSocketTransportInitializerFactory;
