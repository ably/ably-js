...
define(['ably', 'shared_helper', 'async', 'chai'], function (Ably, helper, async, chai) {
  ...
  var publishIntervalHelper = function (currentMessageNum, channel, dataFn, onPublish) {
    return function () {
      ptcb(channel.publish('event0', dataFn()), function () {
        onPublish();
      });
    };
  };
  ...
  describe('realtime/message', function () {
    ...
    it('publishonce', function (done) {
      try {
        ...
        realtime.connection.on('connected', function () {
          ...
          ptcb(rtChannel.attach(), function (err) {
            if (err) {
                ...
                [
                  ...
                  function (cb) {
                    ...
                    var publish = function (i) {
                      ptcb(channel.publish('event', i.toString()), function (err) {
                        try {
                          expect(
                            !err,
    ...
    testOnAllTransports('publishQueued', function (realtimeOpts) {
      return function (done) {
        ...
        try {
          txRealtime = helper.AblyRealtimePromise(utils.mixin(realtimeOpts, { autoConnect: false }));
          ...
          async.series(
            [
              ...
              function (cb) {
                ptcb(rxChannel.attach(), function (err) {
                  cb(err);
                });
              },
              function (cb) {
                async.parallel(
                  [
                    ...
                    function (parCb) {
                      var ackd = 0;
                      var publish = function (i) {
                        ptcb(txChannel.publish('event', { num: i }), function (err) {
                          try {
                            expect(
                              !err,
                              'successfully published ' + i + (err ? ' err was ' + displayError(err) : '')
    ...
    it('publishEcho', function (done) {
      ...
      var rtNoEcho = helper.AblyRealtimePromise({ echoMessages: false }),
        rtEcho = helper.AblyRealtimePromise({ echoMessages: true }),
      ...
      ptcb(rtNoEchoChannel.attach(), function (err) {
        try {
        ...
        ptcb(rtEchoChannel.attach(), function (err) {
          try {
          ...
          ptcb(rtNoEchoChannel.publish('event0', testMsg1), function () {
            ...
            var publishfn = function (cb) {
              ...
              async.eachSeries(
                ...
                function iterator(item, callback) {
                  try {
                    ptcb(restChannel.publish(item), function (err) {
                      try {
    ...
    it('implicit_client_id_0', function (done) {
      var clientId = 'implicit_client_id_0',
        realtime = helper.AblyRealtimePromise({ clientId: clientId });
    ...
    it('explicit_client_id_0', function (done) {
      var clientId = 'explicit_client_id_0',
        ...
        realtime = helper.AblyRealtimePromise({ clientId: clientId, transports: [helper.bestTransport] });
        ...
        ptcb(channel.attach(), function (err) {
          ...
          async.parallel(
            [
              ...
              function (cb) {
                ptcb(channel.publish({ name: 'event0', clientId: clientId }), function (err) {
                  cb(err);
                });
              },
            ],
    ...
    it('explicit_client_id_1', function (done) {
      ...
      ptcb(rest.auth.requestToken({ clientId: clientId }), function (err, token) {
        if (err) {
          done(err);
          return;
        }
        ...
        /* Use a fixed transport as intercepting transport.send */
        var realtime = helper.AblyRealtimePromise({ token: token.token, transports: [helper.bestTransport] }),
        ...
        ptcb(channel.publish({ name: 'event0', clientId: invalidClientId }), function (err) {
          try {
          ...
          function (outercb) {
            async.parallel(
              [
                ...
                function (innercb) {
                  ptcb(channel.publish([{ name: 'a' }, { name: 'b' }, { name: 'c' }, { name: 'd' }]), function (err) {
                    innercb(err);
                  });
                },
              ],
          ...
          function (outercb) {
            async.parallel(
              [
                ...
                function (innercb) {
                  ptcb(channel.publish([{ name: 'a', extras: extras }]), innercb);
                },
              ],
              outercb
            );
    ...
    it.skip('bundling', function (done) {
      var realtime = helper.AblyRealtimePromise({ maxMessageSize: 256, autoConnect: false }),
    ...
    it('subscribes to filtered channel', function (done) {
      ...
      try {
        /* set up realtime */
        var realtime = helper.AblyRealtimePromise({ key: helper.getTestApp().keys[5].keyStr });
        var rest = helper.AblyRestPromise();
        ...
        realtime.connection.on('connected', function () {
          ...
          /* subscribe to event */
          ptcb(rtFilteredChannel.attach(), function (err) {
            if (err) {
              closeAndFinish(done, realtime, err);
              return;
            }
            ...
            ptcb(rtUnfilteredChannel.attach(), function (err) {
              if (err) {
                closeAndFinish(done, realtime, err);
                return;
              }
