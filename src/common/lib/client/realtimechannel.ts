...
class RealtimeChannel extends EventEmitter {
  ...
  async setOptions(options?: API.Types.ChannelOptions): Promise<void> {
  ...
  async detach(): Promise<void> {
    ...
    switch (this.state) {
      ...
      case 'detaching':
        // TODO note that we have to still use the callback version of `once` because we want the event name (can we do something else instead?)
        return new Promise((resolve, reject) => {
          this.once(function (this: { event: string }, stateChange: ChannelStateChange) {
            switch (this.event) {
              case 'detached':
