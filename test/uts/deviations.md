...
### connection_auth: RSA4c1 - errorReason not set on auth failure while CONNECTED

**Spec (RSA4c1/RSA4c3)**: If an auth attempt fails (non-403) while CONNECTED, errorReason should be set with code 80019.

**ably-js behavior**: errorReason is NOT set. The error is caught and logged but not propagated.

**Test**: `RSA4c1/RSA4c3 - authCallback error while CONNECTED sets errorReason`.
