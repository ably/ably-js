...
define(['ably', 'shared_helper', 'async'], function(Ably, helper, async) {
	helper.describeWithCounter('realtime/message', function (expect, counter) {
		var exports = {},
			_exports = {},
		...
		*/
		testOnAllTransports('publishfast', function(realtimeOpts) { return function(done) {
			counter.expect(100);
			try {
						...
						async.parallel([
							function(cb) {
								channel.subscribe('event', function(msg) {
									expect(true, 'Received event ' + msg.data);
									if(msg.data === '49') {
										cb();
									}
								});
							...
							function(cb) {
								...
								var publish = function(i) {
									channel.publish('event', i.toString(), function(err) {
										expect(!err, 'successfully published ' + i + (err ? ' err was ' + displayError(err) : ''));
										ackd++;
										if(ackd === 50) cb();
									});
									if(i < 49) {
		...
		*/
		testOnAllTransports('publishQueued', function(realtimeOpts) { return function(done) {
			counter.expect(150);
			...
			async.series([
					...
					async.parallel([
						function(parCb) {
							...
							rxChannel.subscribe('event', function(msg) {
								...
								expect(true, 'Received event ' + num);
				...
				counter.assert();
					closeAndFinish(done, [rxRealtime, txRealtime]);
				});
			} catch(e) {
				expect(false, 'test failed with exception: ' + e.stack);
				closeAndFinish(done, [rxRealtime, txRealtime]);
		...
		*/
		it('publishEcho', function(done) {
			...
			var finishTest = function() {
				if (receivedMessagesNoEcho.length + receivedMessagesEcho.length == 3) {
					...
					try {
						...
						expect(false, 'Failed to find an expected message in the received messages');
					}
					counter.assert();
			...
			rtNoEchoChannel.attach(function(err) {
				expect(!err,'Attached to rtNoEchoChannel with no error');
				monitorConnection(done, expect, rtNoEcho);
				...
				rtEchoChannel.attach(function(err) {
					expect(!err,'Attached to rtEchoChannel with no error');
					monitorConnection(done, expect, rtEcho);
		...
		it('publishVariations', function(done) {
			...
			counter.expect(testArguments.length * 2);
			try {
				/* set up realtime */
				...
				realtime.connection.on('connected', function() {
					...
					rtChannel.attach(function(err) {
						...
						rtChannel.subscribe(function(msg) {
							...
							switch(msg.name) {
								...
								case 'objectWithNameAndEmptyStringData':
									expect(msg.data).to.equal('', 'Msg data received was a ' + typeof(msg.data) + ' when should have been an empty string');
									break;
								case 'objectWithNameAndFalseData':
								case 'nameAndFalseData':
									expect(msg.data).to.equal(false, 'Msg data received was a ' + typeof(msg.data) + ' when should have been a bool false');
									break;
								case 'nameAndData':
								...
								case undefined:
									...
									break;
								default:
									expect(false, 'Unexpected message ' + msg.name + 'received');
									closeAndFinish(done, realtime);
							}
						...
						async.eachSeries(testArguments, function iterator(args, callback) {
							...
							if(err) {
								expect(false, 'Error received by publish callback ' + displayError(err));
								closeAndFinish(done, realtime);
								return;
							}
						});
					...
					rtChannel.attach(function(err) {
						...
						for(var i = 0; i < testArguments.length; i++) {
							try {
								...
								expect(false, "Exception was not raised");
							} catch (e) {
								expect(true, "Exception correctly raised");
								expect(e.code).to.equal(40013, "Invalid data type exception raised");
							}
						}
						counter.assert();
						closeAndFinish(done, realtime);
		...
		it('publishEncodings', function(done) {
			...
			counter.expect(testArguments.length * 4); // One for sending, one for receiving, one each for data & encoding
			try {
				/* set up realtime */
				var realtime = helper.AblyRealtime();
				var rest = helper.AblyRest();
							...
							rtChannel.subscribe(function(msg) {
								...
								switch(msg.name) {
									case 'justJson':
										...
										expect(msg.encoding).to.equal(null, 'justJson: encoding stripped on decoding');
										break;
									case 'jsonUtf8string':
							...
							async.eachSeries(testArguments, function iterator(item, callback) {
								try {
									restChannel.publish(item, function(err) {
										expect(!err, "Successfully published");
										callback(err);
									});
								} catch (e) {
									expect(false, "Failed to publish");
								}
							}, cb);
						}
		...
		testOnAllTransports('publish', function(realtimeOpts) { return function(done) {
			var count = 10;
			var cbCount = 10;
			...
			realtime.connection.once('connected', function() {
				...
				transport.send = function(message) {
					if (message.action === 15) {
						expect(message.messages[0].name === 'event0', 'Outgoing message interecepted');
						expect(!message.messages[0].clientId, 'client ID is not added by the client library as it is implicit');
					}
		...
		it('explicit_client_id_0', function(done) {
			...
			realtime.connection.once('connected', function() {
				...
				transport.send = function(message) {
					if (message.action === 15) {
						expect(message.messages[0].name === 'event0', 'Outgoing message interecepted');
						expect(message.messages[0].clientId === clientId, 'client ID is present when published to Ably');
					}
					originalSend.apply(transport, arguments);
				};
		...
		it('explicit_client_id_1', function(done) {
			...
			rest.auth.requestToken({ clientId: clientId }, function(err, token) {
				...
				channel.publish({ name: 'event0', clientId: invalidClientId }, function(err) {
					expect(err, 'Message was not published');
					setTimeout(function() { closeAndFinish(done, realtime); }, 500); // ensure that the message is not published
				});
				...
				realtime.connection.connectionManager.on('transport.pending', function(transport) {
					...
					/* subscribe to event */
					channel.subscribe('event0', function(message) {
						expect(false, 'Message should never have been received');
						counter.assert();
					});
				});
			});
					...
					async.parallel([
						function(innercb) {
							...
							channel.subscribe(['a', 'b'], function(message) {
								expect(message.name === 'a' || message.name === 'b', 'Correct messages received');
								++received;
								if(received === 2) {
		...
		* constraints, check they're satisfied */
		it('bundling', function(done) {
			...
			channelOne.subscribe(function(msg) {
				expect(currentName < msg.name, 'Check final ordering preserved');
		...
		it('publishpromise', function(done) {
			...
			var publishPromise = realtime.connection.once('connected').then(function(connectionStateChange) {
				expect(true, 'Check once() returns a promise');
				expect(connectionStateChange.current).to.equal('connected', 'Check promise is resolved with a connectionStateChange');
			...
			var messagePromise = new Promise(function(msgResolve) {
				subscribePromise = channel.subscribe('name', function(msg) {
					...
					expect(true, 'Check subscribe returns a promise that resolves on attach');
				});
			})

			var channelFailedPromise = realtime.channels.get(':invalid').attach()['catch'](function(err) {
				expect(true, 'Check attach returns a promise that is rejected on attach fail');
