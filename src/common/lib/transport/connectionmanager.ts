import { IProtocolMessage, IProtocolMessageConstructor } from 'common/lib/types/protocolmessage';
import * as Utils from 'common/lib/util/utils';
import { IProtocol, IPendingMessage, IPendingMessageConstructor, IProtocolConstructor } from './protocol';
...
export interface IConnectionManager extends EventEmitter {
  ...
  send(msg: IProtocolMessage, queueEvent?: boolean, callback?: ErrCallback): void;
...
export interface IConnectionManagerConstructor {
  new (realtime: Realtime, options: ClientOptions): IConnectionManager;
}
...
const connectionManagerClassFactory = (
  ...
  function bundleWith(dest: IProtocolMessage, src: IProtocolMessage, maxSize: number) {
    let action;
    ...
    const kind = action === actions.PRESENCE ? 'presence' : 'messages',
      ...
      size = messageClass.getMessagesSize(proposed);
    if (size > maxSize) {
      /* RTL6d1 */
      return false;
    }
    ...
    if (
      !Utils.arrEvery(proposed, function (msg: IMessage) {
        return !msg.id;
      })
    ) {
      /* RTL6d7 */
  ...
  class TransportParams implements ITransportParams {
    options: ClientOptions;
    host: string | null;
    mode: string;
    format?: Utils.Format;
  ...
  class ConnectionManager extends EventEmitter implements IConnectionManager {
    realtime: Realtime;
    options: ClientOptions;
    ...
    scheduleTransportActivation(
      ...
      let oldProtocol: IProtocol | null = null;
      ...
      const finishUpgrade = () => {
        ...
        transport.send(
          protocolMessageClass.fromValues({
    ...
    activateTransport(
      ...
      this.activeProtocol = new protocolClass(transport);
      ...
      if (wasActive) {
        Logger.logAction(
          ...
          'Getting, clearing, and requeuing ' +
            (this.activeProtocol as IProtocol).messageQueue.count() +
            ' pending messages'
        );
        this.queuePendingMessages((currentProtocol as IProtocol).getPendingMessages());
        /* Clear any messages we requeue to allow the protocol to become idle.
         * In case of an upgrade, this will trigger an immediate activation of
         * the upgrade transport, so delay a tick so this transport can finish
         * deactivating */
        Platform.Config.nextTick(function () {
          (currentProtocol as IProtocol).clearPendingMessages();
        });
      ...
      const preferenceTimeout = setTimeout(() => {
        ...
        if (!(this.state.state === this.states.connected.state)) {
          Logger.logAction(
            Logger.LOG_MINOR,
            'ConnectionManager.connectPreference()',
            'Shortcircuit connection attempt with ' +
              preference +
              ' failed; clearing preference and trying from scratch'
          );
    ...
    getUpgradePossibilities(): string[] {
      ...
      const current = (this.activeProtocol as IProtocol).getTransport().shortName;
      ...
      switch (this.state.state) {
        case 'connected': {
          ...
          if (
            ...
            const transportParams = (this.activeProtocol as IProtocol).getTransport().params;
            Platform.Config.nextTick(() => {
          ...
          const authMsg = protocolMessageClass.fromValues({
            action: actions.AUTH,
            auth: {
    ...
    send(msg: IProtocolMessage, queueEvent?: boolean, callback?: ErrCallback): void {
      ...
      if (state.sendEvents) {
        ...
        this.sendImpl(new pendingMessageClass(msg, callback));
        return;
      }
      ...
      if (Logger.shouldLog(Logger.LOG_MICRO)) {
        Logger.logAction(
          ...
          'queueing msg; ' + protocolMessageClass.stringify(msg)
        );
      }
      this.queue(msg, callback);
    }

    sendImpl(pendingMessage: IPendingMessage): void {
      ...
      try {
        (this.activeProtocol as IProtocol).send(pendingMessage);
      } catch (e) {
        Logger.logAction(
          Logger.LOG_ERROR,
    ...
    queue(msg: IProtocolMessage, callback: ErrCallback): void {
      ...
      if (lastQueued && !lastQueued.sendAttempted && bundleWith(lastQueued.message, msg, maxSize)) {
        ...
        this.queuedMessages.push(new pendingMessageClass(msg, callback));
      }
    }
    ...
    queuePendingMessages(pendingMessages: Array<IPendingMessage>): void {
    ...
    ping(transport: Transport | null, callback: Function): void {
      ...
      this.ping((this.activeProtocol as IProtocol).getTransport(), onPingComplete);
    }
...
export { connectionManagerClassFactory };
