(function() {
  ...
  var G = {TAction:TAction, TType:TType, TData:R, TPresence:W, TMessage:X, TChannelMessage:Y, TMessageSet:da}, N, Z = function() {
    if(!S) {
      try {
    ...
    if(0 < a) {
      for(var b = 0;b < a;b++) {
        ...
        if(0 < h.pv[0]) {
          var e = H(c);
          if(e) {
            if(la(O[b].swfVersion) && !(h.wk && 312 > h.wk)) {
  ...
  }, q = "undefined", J = "object", pa = "application/x-shockwave-flash", Va = "SWFObjectExprInst", D = window, l = document, P = navigator, $a = !1, ka = [function() {
    if($a) {
  ...
  }], O = [], oa = [], $ = [], ea, na, Ga, Ua, S = !1, ma = !1, K, Ja, Ya = !0, h, mb = typeof l.getElementById != q && typeof l.getElementsByTagName != q && typeof l.createElement != q, qa = P.userAgent.toLowerCase(), ra = P.platform.toLowerCase(), nb = ra ? /win/.test(ra) : /win/.test(qa), ob = ra ? /mac/.test(ra) : /mac/.test(qa), pb = /webkit/.test(qa) ? parseFloat(qa.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : !1, Ka = !+"\v1", fa = [0, 0, 0], A = null;
  if(typeof P.plugins != q && typeof P.plugins["Shockwave Flash"] == J) {
    if((A = P.plugins["Shockwave Flash"].description) && !(typeof P.mimeTypes != q && P.mimeTypes[pa] && !P.mimeTypes[pa].enabledPlugin)) {
  ...
  j.__initialize = function() {
    if(!j.__initialized) {
      ...
      N.embedSWF("swf/WebSocketMainInsecure.swf", "webSocketFlash", "1", "1", "10.0.0", null, null, {hasPriority:!0, swliveconnect:!0, allowScriptAccess:"always"}, null, function(a) {
  ...
  k.inherits(M, u);
  window.FlashTransport = M;
  window.swfobject = N;
  M.isAvailable = function() {
    return xb && N && 10 <= N.getFlashPlayerVersion().major && sa
  };
  M.isAvailable() && (L.availableTransports.flash_socket = M);
  M.tryConnect = function(a, b, c, d) {
    sa.__initialize();
    var g = new M(a, b, c);
    errorCb = function(a) {
      d(a)
  ...
  M.prototype.createWebSocket = function(a, b) {
    var c = 0;
    if(b) {
      for(var d in b) {
        a += (c++ ? "&" : "?") + d + "=" + b[d]
