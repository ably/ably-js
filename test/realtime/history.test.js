  ...
  var parallelPublishMessages = function (done, channel, messages, callback) {
    var publishTasks = utils.arrMap(messages, function (event) {
      return function (publishCb) {
        ptcb(channel.publish(event.name, event.data), publishCb);
      };
    });
          ...
          realtime.connection.whenState('connected', function () {
            ...
            ptcb(rtChannel.attach(), function (err) {
              ...
              parallelPublishMessages(done, restChannel, postAttachMessages, function () {
                ...
                var tests = [
                  function (callback) {
                    ptcb(rtChannel.history(), function (err, resultPage) {
                      if (err) {
                        callback(err);
                      }
                      try {
                  ...
                  function (callback) {
                    ptcb(rtChannel.history({ untilAttach: false }), function (err, resultPage) {
                      if (err) {
                        callback(err);
                      }
                      try {
                  ...
                  function (callback) {
                    ptcb(rtChannel.history({ untilAttach: true }), function (err, resultPage) {
                      if (err) {
                        callback(err);
                      }
