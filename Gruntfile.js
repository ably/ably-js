...
module.exports = function (grunt) {
  ...
  // specifier only, and those resolve identically under both module systems.
  grunt.registerTask(
    ...
    function () {
      ...
      for (const declarationFile of declarationFiles) {
        ...
        // `export =` has no ESM spelling, so an entry point declared that way cannot have an ESM
        // counterpart and does not offer an `import` condition either. react-native-push is the
        // case: the core declares it over a namespace carrying both `create` and the option types,
        // and `export =` is the only form that preserves both.
