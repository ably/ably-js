    ...
    if ('clientId' in normalOptions) {
      if (!(typeof normalOptions.clientId === 'string' || normalOptions.clientId === null)) {
        ...
        throw new ErrorInfo({
          ...
          code: 40012,
          statusCode: 400,
          hint: 'Move "*" out of ClientOptions.clientId. For a wildcard token, set defaultTokenParams: { clientId: "*" } on the client instead. The API key must have wildcard-clientId capability in the Ably dashboard or the server rejects the token request, and if you have the Ably CLI installed, `ably auth keys list` shows the capabilities configured on your key.',
        });
      }
    }
