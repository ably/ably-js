  ...
  describe('realtime/liveobjects', function () {
    ...
    describe('Apply on ACK', () => {
      ...
      describe('Operations applied locally on ACK', function () {
        ...
        for (const scenario of applyOnAckScenarios) {
          it(scenario.description, async function () {
            const helper = this.test.helper;
            const client = RealtimeWithLiveObjects(helper);

            await helper.monitorConnectionThenCloseAndFinishAsync(async () => {
              const channel = client.channels.get(scenario.description, channelOptionsWithObjectModes());
      ...
      describe('Does not double-apply', () => {
        ...
        it('ACK after echo does not double-apply', async function () {
          ...
          await helper.monitorConnectionThenCloseAndFinishAsync(async () => {
            ...
            const echoAppliedPromise = new Promise((resolve) => {
              const { unsubscribe } = counter.subscribe(() => {
                unsubscribe();
                resolve();
              });
      ...
      describe('Does not incorrectly skip operations', () => {
        ...
        it('apply-on-ACK does not update siteTimeserials', async function () {
          ...
          await helper.monitorConnectionThenCloseAndFinishAsync(async () => {
            ...
            // wait for create echo and extract the serial
            await interceptor.waitForEcho();
            helper.recordPrivateApi('read.ProtocolMessage.state');
            const createEcho = interceptor.heldEchoes[0].message;
            // the create has two operations (COUNTER_CREATE and MAP_SET);
            // get the last one's serial, which is what siteTimeserials will be set to
            const createState = createEcho.state[createEcho.state.length - 1];
            const createSerial = createState.serial;
            const siteCode = createState.siteCode;
            ...
            // character to createSerial; this works as long as createSerial is not a prefix
            // of incrementSerial (which we expect since they're from different protocol messages)
            const injectedSerial = createSerial + 'a';

            // verify our assumptions
            expect(injectedSerial > createSerial).to.equal(true, 'injectedSerial > createSerial');
            expect(injectedSerial < incrementSerial).to.equal(true, 'injectedSerial < incrementSerial');

            // inject an operation with this serial
            // if siteTimeserials was NOT updated by apply-on-ACK (still at createSerial level):
            //   injectedSerial > createSerial, so operation is applied
      ...
      describe('ACKs buffered during OBJECT_SYNC', () => {
        it('operation buffered during sync is applied after sync completes', async function () {
          ...
          await helper.monitorConnectionThenCloseAndFinishAsync(async () => {
            ...
            helper.recordPrivateApi('call.makeProtocolMessageFromDeserialized');
            const transport = client.connection.connectionManager.activeProtocol.getTransport();
            const attachedPm = createPM({
              action: 11, // ATTACHED
              channel: channel.name,
              flags: 1 << 7, // HAS_OBJECTS flag
            });
            await transport.onProtocolMessage(attachedPm);
