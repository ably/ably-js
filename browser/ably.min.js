(function() {
  ...
  var G = {TAction:TAction, TType:TType, TData:R, TPresence:W, TMessage:X, TChannelMessage:Y, TMessageSet:da}, M, Z = function() {
    if(!S) {
      try {
    ...
    var d = H(c);
    if(d) {
      "OBJECT" == d.nodeName ? (ea = Ga(d), na = null) : (ea = d, na = c);
      a.id = Va;
  ...
  }], N = [], oa = [], $ = [], ea, na, Fa, Ua, S = !1, ma = !1, K, Ia, Ya = !0, h, ob = typeof l.getElementById != q && typeof l.getElementsByTagName != q && typeof l.createElement != q, qa = O.userAgent.toLowerCase(), ra = O.platform.toLowerCase(), pb = ra ? /win/.test(ra) : /win/.test(qa), qb = ra ? /mac/.test(ra) : /mac/.test(qa), rb = /webkit/.test(qa) ? parseFloat(qa.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : !1, Ja = !+"\v1", fa = [0, 0, 0], A = null;
  if(typeof O.plugins != q && typeof O.plugins["Shockwave Flash"] == J) {
    if((A = O.plugins["Shockwave Flash"].description) && !(typeof O.mimeTypes != q && O.mimeTypes[pa] && !O.mimeTypes[pa].enabledPlugin)) {
      $a = !0, Ja = !1, A = A.replace(/^.*\s+(\S+\s+\S+$)/, "$1"), fa[0] = parseInt(A.replace(/^(.*)\..*$/, "$1"), 10), fa[1] = parseInt(A.replace(/^.*\.(.*)\s.*$/, "$1"), 10), fa[2] = /[a-zA-Z]/.test(A) ? parseInt(A.replace(/^.*[a-zA-Z]+(.*)$/, "$1"), 10) : 0
    }
  }else {
    if(typeof D.ActiveXObject != q) {
      try {
        ...
        if(ab && (A = ab.GetVariable("$version"))) {
          Ja = !0, A = A.split(" ")[1].split(","), fa = [parseInt(A[0], 10), parseInt(A[1], 10), parseInt(A[2], 10)]
        }
      }catch(Gb) {
      }
    }
  }
  h = {w3:ob, pv:fa, wk:rb, ie:Ja, win:pb, mac:qb};
  ...
  M = {registerObject:function(a, b, c, f) {
    ...
    if(ma) {
      var a = H(Va);
      a && ea && (a.parentNode.replaceChild(ea, a), na && (T(na, !0), h.ie && h.win && (ea.style.display = "block")), Fa && Fa(Ua));
      ma = !1
    }
  }};
