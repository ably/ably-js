...
define(['ably', 'shared_helper', 'async'], function(Ably, helper, async) {
	helper.describeWithCounter('realtime/sync', function (expect, counter) {
		var exports = {},
			_exports = {},
		...
		it('setupSync', function(done) {
			counter.expect(1);
			helper.setupApp(function(err) {
				expect(!err, 'app set up ' + (err && displayError(err)));
				counter.assert();
				done();
			});
		});
			...
			async.series([
				...
				function(cb) {
					channel.presence.get(function(err, results) {
						expect(results.length).to.equal(2, 'Check correct number of results');
						expect(channel.presence.syncComplete, 'Check in sync');
						expect(extractClientIds(results)).to.deep.equal(['one', 'three'], 'check two has gone and three is there');
						cb(err);
					});
				}
			], function(err) {
		...
		*/
		it('presence_sync_interruptus', function(done) {
			...
			async.series([
				...
				counter.assert();
				closeAndFinish(done, [enterer, syncer]);
			});
		});
	});
});
