 ...
 * @spec OMO1
 */
export interface ObjectsMapOp<T> {
 ...
 * @spec OME1
 */
export interface ObjectsMapEntry<T> {
...
export interface ObjectsMap<T> {
  ...
  entries?: Record<string, ObjectsMapEntry<T>>; // OMP3b
}
 ...
 * @spec OOP1
 */
export interface ObjectOperation<T> {
...
export interface ObjectState<T> {
   ...
   */
  createOp?: ObjectOperation<T>; // OST2d
  /**
...
function encode(
  ...
  const result: WireObjectMessage = Object.assign(new WireObjectMessage(utils, messageEncoding), copyMsg(message));
...
export class WireObjectMessage {
  ...
  decode(client: BaseClient, format: Utils.Format | undefined): ObjectMessage {
    ...
    const result: ObjectMessage = Object.assign(new ObjectMessage(this._utils, this._messageEncoding), copyMsg(this));
