...
export class LiveObjects {
  ...
  private _applySync(): void {
    ...
    for (const [objectId, entry] of this._syncLiveObjectsDataPool.entries()) {
      ...
      switch (objectType) {
        case 'LiveCounter':
          newObject = new LiveCounter(this, regionalTimeserialObj, entry.created, entry.objectData, objectId);
          break;

        case 'LiveMap':
          newObject = new LiveMap(this, regionalTimeserialObj, entry.semantics, entry.objectData, objectId);
          break;
