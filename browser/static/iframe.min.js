...
return c}},A=function(){function a(){}a.addListener=function(a,b,f){a.addEventListener?a.addEventListener(b,f,!1):a.attachEvent("on"+b,function(){f.apply(a,arguments)})};a.removeListener=function(a,b,f){a.removeEventListener?a.removeEventListener(b,f,!1):a.detachEvent("on"+b,function(){f.apply(a,arguments)})};a.addMessageListener=function(c,b){a.addListener(c,"message",b)};a.removeMessageListener=function(c,b){a.removeListener(c,"message",b)};a.addUnloadListener=function(c){a.addListener(window,"unload",
c)};return a}(),x=function(){function a(){for(var a in e)e[a].dispose()}function c(){return window.XMLHttpRequest&&"withCredentials"in new XMLHttpRequest?p=!0:s&&document.domain&&"https:"==window.location.protocol?!0:!1}function b(a,b,c,h,d){z.call(this);c=c||{};c.rnd=String(Math.random()).substr(2);this.uri=a+m.toQueryString(c);this.headers=b||{};this.body=h;this.requestMode=d;this.requestComplete=!1;e[this.id=String(++g)]=this}function f(a,c,h,d,e){h.ua="xdr";b.call(this,a,c,h,d,e)}var d=function(){},
g=0,e={},s=window.XDomainRequest,p;m.inherits(b,z);b.isAvailable=c;var h=b.createRequest=function(a,c,h,d,e){return p?new b(a,c,h,d,e):new f(a,c,h,d,e)};b.prototype.complete=function(a,b,c,h){this.requestComplete||(this.requestComplete=!0,b&&this.emit("data",b),this.emit("complete",a,b,c,h),this.dispose())};b.prototype.abort=function(){this.dispose()};b.prototype.exec=function(){function a(){r=f.responseText;for(var b=r.length-1,c,h;v<b&&-1<(c=r.indexOf("\n",v));){h=r.slice(v,c);v=c+1;a:{try{h=JSON.parse(h)}catch(d){h=
...
(e["content-type"]="application/json"))&&"string"!=typeof h&&(h=JSON.stringify(h));f.open(d,this.uri,!0);f.responseType=p;f.withCredentials="true";for(var u in e)f.setRequestHeader(u,e[u]);var l=f.onerror=function(a){a.code=8E4;g.complete(a)};f.onabort=function(){var a=Error("Request cancelled");a.statusCode=400;l(a)};f.ontimeout=function(){var a=Error("Request timed out");a.statusCode=408;l(a)};var t,q,r,w,v=0,B=!1;f.onreadystatechange=function(){var h=f.readyState;if(!(3>h)&&0!==f.status)if(void 0===
q&&(q=f.status,1223===q&&(q=204),clearTimeout(c),w=400>q,204==q?g.complete():t=3==g.requestMode&&w),3==h&&t)a();else if(4==h)if(t)b();else a:{try{var d=f.getResponseHeader&&f.getResponseHeader("Content-Type"),e=d?"application/json"==d:"text"==f.responseType;r=e?f.responseText:f.response;if(!r){204!=status&&(l=Error("Incomplete response body from server"),l.statusCode=400,g.complete(l));break a}e&&(r=JSON.parse(String(r)),B=!0)}catch(k){var l=Error("Malformed response body from server: "+k.message);
l.statusCode=400;g.complete(l);break a}w?g.complete(null,r,d&&{"Content-Type":d},B):(l=r.error,l||(l=Error("Error response received from server: "+q),l.statusCode=q),g.complete(l))}};f.send(h)};b.prototype.dispose=function(){var a=this.xhr;if(a){a.onreadystatechange=a.onerror=a.onabort=a.ontimeout=d;this.xhr=null;var b=this.timer;b&&(clearTimeout(b),this.timer=null);this.requestComplete||a.abort()}delete e[this.id]};m.inherits(f,b);f.prototype.exec=function(){function a(){clearTimeout(h);if(l=e.responseText){var b=
l.length-1;if("\n"==l[b]||(b=-1<l.indexOf("\n"))){var c=l.slice(0,b);try{var c=JSON.parse(c),d=c.error;if(d)u=d.statusCode||500,g.complete(d);else if(u=l?201:200,s=3==g.requestMode)t=b,m.isEmpty(c)||g.emit("data",c)}catch(f){d=Error("Malformed response body from server: "+f.message),d.statusCode=400,g.complete(d)}}}}function b(){l=e.responseText;for(var a=l.length-1,c,h;t<a&&-1<(c=l.indexOf("\n",t));){h=l.slice(t,c);t=c+1;a:{try{h=JSON.parse(h)}catch(d){err=Error("Malformed response body from server: "+
...
e.onabort=function(){k.logAction(k.LOG_ERROR,"Request.onabort()","");var a=Error("Request cancelled");a.statusCode=400;p(a)};e.ontimeout=function(){k.logAction(k.LOG_ERROR,"Request.timeout()","");var a=Error("Request timed out");a.statusCode=408;p(a)};var s,u,l,t=0;e.onprogress=function(){void 0===u?a():s&&b()};e.onload=function(){if(void 0===u&&(a(),g.requestComplete))return;if(s)c();else a:{try{l=e.responseText;if(!l||!l.length){204!=status&&(h=Error("Incomplete response body from server"),h.statusCode=
...
this.pendingCallback=this.recvRequest=this.sendRequest=null;var a=this;A.addMessageListener(window,function(b){a.onMessageEvent(b.data)})}var f=location.origin||location.protocol+"//"+location.hostname+(location.port?":"+location.port:""),d=m.parseQueryString(window.location.search),g=d.origin;delete d.origin;var e="access_token"in d?{access_token:d.access_token}:{key_id:d.key_id,key_value:d.key_value},n=window.parent,p=s.Action;b.prototype.connect=function(){var a=(this.baseUri=f+"/comet/")+"connect",
...
b.prototype.send=function(a,b){k.logAction(k.LOG_MICRO,"IframeAgent.send()","msg = "+JSON.stringify(a));if(this.sendRequest)this.pendingItems=this.pendingItems||[],this.pendingItems.push(a),this.pendingCallback=this.pendingCallback||C(),this.pendingCallback.push(b);else{var c=this.pendingItems||[];c.push(a);this.pendingItems=null;var d=this.pendingCallback;d&&(d.push(b),b=d,this.pendingCallback=null);this.sendItems(c,b)}};b.prototype.sendItems=function(b,c){var d=this.sendUri,f=this;d?(d=this.sendRequest=
x.createRequest(d,null,e,a(b),0),d.on("complete",function(a,b){a&&k.logAction(k.LOG_ERROR,"IframeAgent.sendItems()","on complete: err = "+a);f.sendRequest=null;if(b)f.onData(b);var d=f.pendingItems;if(d){f.pendingItems=null;var e=f.pendingCallback;f.pendingCallback=null;m.nextTick(function(){f.sendItems(d,e)})}c(a)}),d.exec()):c({message:"Unable to send; not connected",code:8E4,statusCode:400})};b.prototype.recv=function(){if(!this.recvRequest&&this.isConnected){var a=this,b=this.recvRequest=x.createRequest(this.recvUri,
null,e,null,a.stream?3:2);b.on("data",function(b){a.onData(b)});b.on("complete",function(b){a.recvRequest=null;b?a.postErrorEvent(b):m.nextTick(function(){a.recv()})});b.exec()}};b.prototype.onData=function(a){this.postMessageEvent(a)};b.prototype.postMessageEvent=function(b){n.postMessage(a(b),g)};b.prototype.postErrorEvent=function(a,b){var c=b;if(a){var d=new s.fromValues({action:p.ERROR,error:a});c&&m.mixin(d,c);c=d}this.postMessageEvent([c])};(new b).connect()})();"undefined"!==typeof Realtime&&
(y.Rest=Rest,y.Realtime=Realtime,Realtime.ConnectionManager=ConnectionManager,Realtime.BufferUtils=Rest.BufferUtils=BufferUtils,"undefined"!==typeof Crypto&&(Realtime.Crypto=Rest.Crypto=Crypto),Realtime.Message=Rest.Message=Message,Realtime.PresenceMessage=Rest.PresenceMessage=PresenceMessage,Realtime.ProtocolMessage=Rest.ProtocolMessage=s)}).call({});
