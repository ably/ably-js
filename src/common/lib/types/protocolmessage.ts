...
import { IMessage, IMessageConstructor } from './message';
import { IPresenceMessage, IPresenceMessageConstructor } from './presencemessage';
...
export interface IProtocolMessageConstructor {
  new (): IProtocolMessage;
  fromValues(values: unknown): IProtocolMessage;
  stringify(msg: any): string;
  deserialize(serialized: unknown, format?: Utils.Format): IProtocolMessage;
  serialize(body: unknown, format?: Utils.Format): string | Buffer;
  fromDeserialized(deserialized: Record<string, unknown>): IProtocolMessage;
  Action: typeof actions;
  channelModes: string[];
}

const protocolMessageClassFactory = (
  messageClass: IMessageConstructor,
  presenceMessageClass: IPresenceMessageConstructor
): IProtocolMessageConstructor => {
  return class ProtocolMessage implements IProtocolMessage {
    action?: number;
    flags?: number;
    id?: string;
    timestamp?: number;
    ...
    static fromDeserialized = function (deserialized: Record<string, unknown>): ProtocolMessage {
      ...
      const messages = deserialized.messages as IMessage[];
      if (messages) for (let i = 0; i < messages.length; i++) messages[i] = messageClass.fromValues(messages[i]);
      const presence = deserialized.presence as IPresenceMessage[];
      if (presence)
        for (let i = 0; i < presence.length; i++) presence[i] = presenceMessageClass.fromValues(presence[i], true);
    ...
    static stringify = function (msg: any): string {
      ...
      if (msg.messages) result += '; messages=' + toStringArray(messageClass.fromValuesArray(msg.messages));
      if (msg.presence) result += '; presence=' + toStringArray(presenceMessageClass.fromValuesArray(msg.presence));
...
export { protocolMessageClassFactory };
