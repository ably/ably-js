...
exports.setup = function(base) {
	...
	function resume_inactive(test, channelName, txOpts, rxOpts) {
		...
		var txRealtime = base.realtime(mixin(txOpts, {
			//log: {level: 4},
			key: base.testVars.testAppId + '.' + base.testVars.testKey0Id + ':' + base.testVars.testKey0.value
		}));
		var rxRealtime = base.realtime(mixin(rxOpts, {
			//log: {level: 4},
			key: base.testVars.testAppId + '.' + base.testVars.testKey0Id + ':' + base.testVars.testKey0.value
		}));
		test.expect(3);
	...
	rExports.resume_inactive_ws_text = function(test) {
	...
	rExports.resume_inactive_ws_binary = function(test) {
	...
	function resume_active(test, channelName, txOpts, rxOpts) {
		...
		function phase1(callback) {
			/* subscribe to event */
			rxChannel.subscribe('event0', function(msg) {
				//console.log('subscribe callback called: msg = ' + msg);
				++rxCount;
			});
			var txCount = 0
			function ph1TxOnce() {
	...
	rExports.resume_active_ws_text = function(test) {
	...
	rExports.resume_active_ws_binary = function(test) {
