...
export class Objects {
   ...
   * @internal
   */
  onAttached(hasObject?: boolean): void {
    this._client.Logger.logAction(
      this._client.logger,
      this._client.Logger.LOG_MINOR,
      'Objects.onAttached()',
      `channel=${this._channel.name}, hasObject=${hasObject}`,
    );

    const fromInitializedState = this._state === ObjectsState.initialized;
    if (hasObject || fromInitializedState) {
      // should always start a new sync sequence if we're in the initialized state, no matter the HAS_OBJECT flag value.
      // this guarantees we emit both "syncing" -> "synced" events in that order.
      this._startNewSync();
    }

    if (!hasObject) {
      // if no HAS_OBJECT flag received on attach, we can end sync sequence immediately and treat it as no objects on a channel.
   ...
   * @internal
   */
  actOnChannelState(state: API.ChannelState, hasObject?: boolean): void {
    switch (state) {
      case 'attached':
        this.onAttached(hasObject);
        break;
