import { IProtocolMessageConstructor, IProtocolMessage } from '../types/protocolmessage';
...
export interface ITransport extends EventEmitter {
  ...
  onProtocolMessage(message: IProtocolMessage): void;
...
const transportClassFactory = (protocolMessageClass: IProtocolMessageConstructor): ITransportConstructor => {
  ...
  abstract class Transport extends EventEmitter implements ITransport {
    ...
    abstract send(message: IProtocolMessage): void;
      ...
      if (Logger.shouldLog(Logger.LOG_MICRO)) {
        Logger.logAction(
          ...
          'received on ' +
            this.shortName +
            ': ' +
            protocolMessageClass.stringify(message) +
            '; connectionId = ' +
    ...
    onDisconnect(message: IProtocolMessage): void {
    ...
    onFatalError(message: IProtocolMessage): void {
    ...
    onClose(message: IProtocolMessage): void {
      const err = message && message.error;
    ...
    ping(id: string): void {
      const msg: Record<string, number | string> = { action: protocolMessageClass.Action.HEARTBEAT };
      if (id) msg.id = id;
      this.send(protocolMessageClass.fromValues(msg));
    }
