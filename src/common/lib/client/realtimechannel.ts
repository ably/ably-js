...
class RealtimeChannel extends EventEmitter {
  ...
  async processMessage(message: ProtocolMessage): Promise<void> {
    ...
    switch (message.action) {
      case actions.ATTACHED: {
        ...
        const hasObject = message.hasFlag('HAS_OBJECT');
        if (this.state === 'attached') {
          if (!resumed) {
            ...
            if (this._objects) {
              this._objects.onAttached(hasObject);
            }
          }
          ...
          this.notifyState('attached', message.error, resumed, hasPresence, hasBacklog, hasObject);
        }
        break;
      }
  ...
  notifyState(
    ...
    if (this._objects) {
      this._objects.actOnChannelState(state, hasObject);
    }
