(function() {
  ...
  var F = {TAction:TAction, TType:TType, TData:S, TPresence:T, TMessage:U, TChannelMessage:aa, TMessageSet:ga}, qa = function() {
  };
  ...
  var V, ba = function() {
    ...
    var d = K(c);
    if(d) {
      "OBJECT" == d.nodeName ? (ha = Qa(d), va = null) : (ha = d, va = c);
      a.id = fb;
  ...
  }], O = [], wa = [], ca = [], ha, va, Pa, eb, W = !1, ua = !1, N, Sa, ib = !0, h, zb = typeof j.getElementById != s && typeof j.getElementsByTagName != s && typeof j.createElement != s, ya = P.userAgent.toLowerCase(), za = P.platform.toLowerCase(), Ab = za ? /win/.test(za) : /win/.test(ya), Bb = za ? /mac/.test(za) : /mac/.test(ya), Cb = /webkit/.test(ya) ? parseFloat(ya.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : !1, Ta = !+"\v1", ia = [0, 0, 0], D = null;
  if(typeof P.plugins != s && typeof P.plugins["Shockwave Flash"] == M) {
    if((D = P.plugins["Shockwave Flash"].description) && !(typeof P.mimeTypes != s && P.mimeTypes[xa] && !P.mimeTypes[xa].enabledPlugin)) {
      kb = !0, Ta = !1, D = D.replace(/^.*\s+(\S+\s+\S+$)/, "$1"), ia[0] = parseInt(D.replace(/^(.*)\..*$/, "$1"), 10), ia[1] = parseInt(D.replace(/^.*\.(.*)\s.*$/, "$1"), 10), ia[2] = /[a-zA-Z]/.test(D) ? parseInt(D.replace(/^.*[a-zA-Z]+(.*)$/, "$1"), 10) : 0
    }
  }else {
    if(typeof J.ActiveXObject != s) {
      try {
        ...
        if(lb && (D = lb.GetVariable("$version"))) {
          Ta = !0, D = D.split(" ")[1].split(","), ia = [parseInt(D[0], 10), parseInt(D[1], 10), parseInt(D[2], 10)]
        }
      }catch(Ob) {
      }
    }
  }
  h = {w3:zb, pv:ia, wk:Cb, ie:Ta, win:Ab, mac:Bb};
  ...
  V = {registerObject:function(a, b, c, d) {
    ...
    if(ua) {
      var a = K(fb);
      a && ha && (a.parentNode.replaceChild(ha, a), va && (X(va, !0), h.ie && h.win && (ha.style.display = "block")), Pa && Pa(eb));
      ua = !1
    }
  }};
  var Aa;
  ...
  p.prototype.queue = function(a, b) {
    ...
    c && Va.mergeTo(c.msg, a) ? (c.merged || (c.callback = new ja([c.callback]), c.merged = !0), c.listener.push(b)) : this.queuedMessages.push(new ob(a, b))
  };
  ...
  var u = function(a, b, c) {
    ...
    this.auth = b;
    this.params = c;
    c.binary ? (this.thriftTransport = ka.TTransport, this.thriftProtocol = ka.TBinaryProtocol, this.protocolBuffer = new ka.CheckedBuffer(Fb)) : (this.thriftTransport = ka.TStringTransport, this.thriftProtocol = ka.TJSONProtocol);
    this.isConnected = !1
  }, Gb = "object" == typeof window, la = ("object" == typeof F ? F : require("../nodejs/lib/protocol/clientmessage_types")).TAction, ka = Gb ? d : require("thrift"), Fb = 1024;
  n.inherits(u, z);
  ...
  w.prototype.send = function(a, b) {
    if(this.sendRequest) {
      this.pendingMessage = this.pendingMessage || new Xa.TMessageSet({items:[]}), this.pendingMessage.items.push(a), this.pendingCallback = this.pendingCallback || new ja, this.pendingCallback.push(b)
    }else {
  ...
  Y.prototype.publish = function(a, b, c) {
    ...
    ma.post(d, "/channels/" + this.name + "/publish", a, g, null, c)
  };
  var Va, y = function(a, b, c) {
    ...
    this.options = n.prototypicalClone(Lb, c);
    this.state = "initialized";
    this.subscriptions = new z;
    this.pendingEvents = []
  }, I = "object" == typeof F ? F : require("../nodejs/lib/protocol/clientmessage_types"), R = I.TAction, ea = function() {
  }, Lb = {queueEvents:!0};
  n.inherits(y, Y);
  y.prototype.publish = function(a, b, c) {
    e.logAction(e.LOG_MICRO, "RealtimeChannel.publish()", "name = " + a);
    var c = c || ea, d = this.connectionManager.state;
    G.activeState(d) ? (d = new I.TMessage, d.name = a, d.data = oa.createPayload(b), "attached" == this.state ? (e.logAction(e.LOG_MICRO, "RealtimeChannel.publish()", "sending message"), a = new I.TChannelMessage, a.action = I.TAction.MESSAGE, a.channel = this.name, a.messages = [d], this.sendMessage(a, c)) : ("pending" != this.state && this.attach(), e.logAction(e.LOG_MICRO, "RealtimeChannel.publish()", "queueing message"), this.pendingEvents.push({message:d, listener:c}))) : c(d.defaultMessage)
  };
  ...
  y.prototype.attach = function(a) {
    var a = a || ea, b = this.connectionManager, c = b.state;
  ...
  y.prototype.subscribe = function() {
    ...
    var b = a[0], c = a[1], a = a[2] = a[2] || ea, d = this.subscriptions;
      ...
      case R.MESSAGE:
        if(a = a.messages) {
          for(var b = Array(a.length), c = 0;c < b.length;c++) {
            var d = a[c];
            b[c] = new oa(d.channelSerial, d.timestamp, d.name, oa.getPayload(d.data))
          }
          this.onEvent(b)
        }
        break;
  ...
  y.prototype.setAttached = function(a) {
    ...
    try {
      if(this.pendingEvents.length) {
        var b = new I.TChannelMessage({action:I.TAction.MESSAGE, channel:this.name, messages:[]}), c = new ja;
  ...
  L.prototype.setPresence = function(a, b) {
    ...
    for(var c = 0;c < a.length;c++) {
      ...
      "leave" == d.state ? delete this.clients[h] : g = this.clients[h] = oa.getPayload(d.clientData);
  ...
  ub = L;
  var oa, Za = function(a, b, c, d) {
    this.channelSerial = a;
    this.timestamp = b;
    this.name = c;
    this.data = d
  }, C = "object" == typeof F ? F : require("../nodejs/lib/protocol/clientmessage_types"), Mb = {"[object Null]":function(a) {
    a.type = C.TType.NONE;
    return!0
    ...
    var c = Mb[Object.prototype.toString.call(b)];
    return c && c(a, b)
  }};
  Za.createPayload = function(a) {
    var b = new C.TData, c = Nb[typeof a];
    if(c && c(b, a)) {
      return b
    }
    throw Error("Unsupported data type: " + Object.prototype.toString.call(a));
  };
  Za.getPayload = function(a) {
    var b = void 0;
    switch(a.type) {
      case 1:
        b = !0;
