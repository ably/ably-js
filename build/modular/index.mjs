...
var channelModes = ["PRESENCE", "PUBLISH", "SUBSCRIBE", "PRESENCE_SUBSCRIBE", "ANNOTATION_PUBLISH", "ANNOTATION_SUBSCRIBE"];
...
var actions3 = ["message.create", "message.update", "message.delete", "meta.occupancy", "message.summary"];
...
var RestAnnotations = class {
  ...
  async publish(refSerial, refType, data) {
    const annotation = annotation_default.fromValues({
    ...
    await resource_default.post(client, basePathForSerial(this.channel, refSerial), requestBody, headers, params, null, true);
  }
  async get(serial, params) {
    const client = this.channel.client, format = client.options.useBinaryProtocol ? "msgpack" /* msgpack */ : "json" /* json */, envelope = client.http.supportsLinkHeaders ? void 0 : format, headers = defaults_default.defaultGetHeaders(client.options, {
      format
    });
    mixin(headers, client.options.headers);
    return new paginatedresource_default(client, basePathForSerial(this.channel, serial), headers, envelope, async (body, _, unpacked) => {
      const decoded = unpacked ? body : decodeBody(body, client._MsgPack, format);
...
var RealtimeAnnotations = class {
  ...
  async subscribe(..._args) {
    ...
    if ((this.channel._mode & flags.ANNOTATION_SUBSCRIBE) === 0) {
      throw new ErrorInfo("You're trying to add an annotation listener, but you haven't requested the annotation_subscribe channel mode in ChannelOptions, so this won't do anything (we only deliver annotations to clients who have explicitly requested them)", 93001, 400);
    }
    this.subscriptions.on(event, listener);
  }
  unsubscribe(..._args) {
...
var Protocol = class extends eventemitter_default {
  ...
  send(pendingMessage) {
    ...
    if (this.logger.shouldLog(logger_default.LOG_MICRO)) {
      logger_default.logActionNoStrip(this.logger, logger_default.LOG_MICRO, "Protocol.send()", "sending msg; " + stringify(pendingMessage.message, this.transport.connectionManager.realtime._RealtimePresence, this.transport.connectionManager.realtime._Annotations));
    }
...
var Transport = class extends eventemitter_default {
  ...
  onProtocolMessage(message) {
    if (this.logger.shouldLog(logger_default.LOG_MICRO)) {
      logger_default.logActionNoStrip(this.logger, logger_default.LOG_MICRO, "Transport.onProtocolMessage()", "received on " + this.shortName + ": " + stringify(message, this.connectionManager.realtime._RealtimePresence, this.connectionManager.realtime._Annotations) + "; connectionId = " + this.connectionManager.connectionId);
    }
    this.onActivity();
    switch (message.action) {
    ...
    try {
      ...
      if (items && items.length)
        for (let i = 0; i < items.length; i++)
          this.onProtocolMessage(fromDeserialized(items[i], this.connectionManager.realtime._RealtimePresence, this.connectionManager.realtime._Annotations));
    } catch (e) {
...
var WebSocketTransport = class extends transport_default {
  ...
  onWsData(data) {
    try {
      this.onProtocolMessage(deserialize(data, this.connectionManager.realtime._MsgPack, this.connectionManager.realtime._RealtimePresence, this.connectionManager.realtime._Annotations, this.format));
    } catch (e) {
