...
export interface IRealtimePresence {
  channel: IRealtimeChannel;
  onAttached(hasPresence?: boolean): void;
  setPresence(presenceSet: IPresenceMessage[], isSync: boolean, syncChannelSerial?: string): void;
  actOnChannelState(state: string, hasPresence?: boolean, err?: ErrorInfo | null): void;
}

export interface IRealtimePresenceConstructor {
  new (channel: IRealtimeChannel): IRealtimePresence;
}

function getClientId(realtimePresence: IRealtimePresence) {
  return realtimePresence.channel.realtime.auth.clientId;
}

function isAnonymousOrWildcard(realtimePresence: IRealtimePresence) {
...
function newerThan(item: IPresenceMessage, existing: IPresenceMessage) {
...
const realtimePresenceClassFactory = (
  superclass: IPresenceConstructor,
  presenceMessageClass: IPresenceMessageConstructor
) => {
  class RealtimePresence extends superclass implements IRealtimePresence {
    channel: IRealtimeChannel;
    pendingPresence: { presence: IPresenceMessage; callback: ErrCallback }[];
    ...
    _enterOrUpdateClient(
      ...
      const presence = presenceMessageClass.fromValues({
        action: action,
        data: data,
      });
      if (id) {
      ...
      presenceMessageClass.encode(presence, channel.channelOptions as CipherOptions, (err: IPartialErrorInfo) => {
        if (err) {
          callback(err);
          return;
        }
    ...
    get(this: RealtimePresence, params: RealtimePresenceParams, callback: StandardCallback<IPresenceMessage[]>): any {
      const args = Array.prototype.slice.call(arguments);
      if (args.length == 1 && typeof args[0] == 'function') args.unshift(null);

      params = args[0] as RealtimePresenceParams;
      callback = args[1] as StandardCallback<IPresenceMessage[]>;
    ...
    history(
      ...
      superclass.prototype._history.call(this, params, callback);
    }
    ...
    setPresence(presenceSet: IPresenceMessage[], isSync: boolean, syncChannelSerial?: string): void {
      ...
      for (let i = 0; i < presenceSet.length; i++) {
        const presence = presenceMessageClass.fromValues(presenceSet[i]);
    ...
    _synthesizeLeaves(items: IPresenceMessage[]): void {
  ...
  class PresenceMap extends EventEmitter {
    map: Record<string, IPresenceMessage>;
    residualMembers: Record<string, IPresenceMessage> | null;
    syncInProgress: boolean;
    presence: RealtimePresence;
    memberKey: (item: IPresenceMessage) => string;

    constructor(presence: RealtimePresence, memberKey: (item: IPresenceMessage) => string) {
      super();
    ...
    put(item: IPresenceMessage) {
      if (item.action === 'enter' || item.action === 'update') {
        item = presenceMessageClass.fromValues(item);
        item.action = 'present';
      }
      const map = this.map,
    ...
    endSync() {
      ...
      if (syncInProgress) {
        ...
        this.presence._synthesizeLeaves(Utils.valuesArray(this.residualMembers as Record<string, IPresenceMessage>));
...
export { realtimePresenceClassFactory };
