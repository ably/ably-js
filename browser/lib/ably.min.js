(function() {
  for(var lb = ["web_socket", "flash_socket", "xhr", "jsonp"], ma = {disconnected:{statusCode:408, code:80003, reason:"Connection to server temporarily unavailable"}, suspended:{statusCode:408, code:80002, reason:"Connection to server unavailable"}, failed:{statusCode:408, code:8E4, reason:"Connection failed or disconnected by server"}}, Ba = function(a, b, c) {
    function f() {
    }
  ...
  X.MAX_INT = Math.pow(2, 53);
  X.MIN_INT = -Math.pow(2, 53);
  X.prototype = {_2scomp:function() {
    ...
    for(var b = this.buffer, c = this.offset, f = b[0] & 128, h = 0, d = 1, e = 7, l = 1;0 <= e;e--, l *= 256) {
      var m = b[c + e];
      f && (m = (m ^ 255) + d, d = m >> 8, m &= 255);
      h += m * l
    }
    return!a && h >= X.MAX_INT ? f ? -Infinity : Infinity : f ? -h : h
  }, valueOf:function() {
  ...
  D.prototype = {getArray:function() {
    ...
    var a = a || 0, b = b || this.length, c = new D, f = c.length = b - a, h = c.offset = this.offset + a, d = c.buf = this.buf;
  ...
  k.prototype.writeI64 = function(a) {
    a.buffer ? this.trans.writeI64(a) : this.trans.writeI64(new X(a))
  };
  ...
  var nb = new D(0), Y = d.TTransport = function(a, b) {
    this.buf = a || nb;
    this.onFlush = b;
    this.reset()
  };
  ...
  Ea.receiver = function(a) {
    ...
    return function(d) {
      ...
      for(;d.length;) {
        ...
        d.length >= b ? (d.copy(f, c, 0, b), d = d.slice(b, d.length), b = 0, a(new Ea(f))) : d.length && (d.copy(f, c, 0, d.length), b -= d.length, c += d.length, d = d.slice(d.length, d.length))
      }
    }
  };
  Ba(Ea, Y, {flush:function() {
    var a = this;
      ...
      switch(b.fid) {
        ...
        case 15:
          if(c == d.Type.SET) {
            ...
            for(c = 0;c < b;++c) {
              f = null, f = new Z, f.read(a), this.presence.push(f)
            }
            a.readSetEnd()
          }else {
            a.skip(c)
  ...
  var H = {TAction:TAction, TType:TType, TData:R, TPresence:Z, TMessage:$, TChannelMessage:aa, TMessageSet:fa}, S, ba = function() {
    if(!T) {
      try {
        ...
        if(0 < g.pv[0]) {
          ...
          if(e) {
            if(oa(N[b].swfVersion) && !(g.wk && 312 > g.wk)) {
              U(c, !0), f && (d.success = !0, d.ref = Fa(c), f(d))
            }else {
              if(N[b].expressInstall && Ga()) {
                ...
                for(var p = {}, e = e.getElementsByTagName("param"), l = e.length, m = 0;m < l;m++) {
                  "movie" != e[m].getAttribute("name").toLowerCase() && (p[e[m].getAttribute("name")] = e[m].getAttribute("value"))
                }
                Ha(d, p, c, f)
              }else {
                ob(e), f && f(d)
    ...
    if(d) {
      if(typeof a.id == r && (a.id = c), g.ie && g.win) {
        ...
        p = "";
        for(var l in b) {
          b[l] != Object.prototype[l] && (p += '<param name="' + l + '" value="' + b[l] + '" />')
        }
        ...
        l = j.createElement(K);
        l.setAttribute("type", sa);
        for(var m in a) {
          a[m] != Object.prototype[m] && ("styleclass" == m.toLowerCase() ? l.setAttribute("class", a[m]) : "classid" != m.toLowerCase() && l.setAttribute(m, a[m]))
        }
        for(e in b) {
          b[e] != Object.prototype[e] && "movie" != e.toLowerCase() && (a = l, p = e, m = b[e], c = j.createElement("param"), c.setAttribute("name", p), c.setAttribute("value", m), a.appendChild(c))
        }
    ...
    if(!g.ie || !g.mac) {
      ...
      if(d) {
        ...
        f && (Ma = L = null);
        if(!L || Ma != c) {
          f = j.createElement("style"), f.setAttribute("type", "text/css"), f.setAttribute("media", c), L = d.appendChild(f), g.ie && (g.win && typeof j.styleSheets != r && 0 < j.styleSheets.length) && (L = j.styleSheets[j.styleSheets.length - 1]), Ma = c
        }
    ...
    if($a) {
      ...
      T && I(a) ? I(a).style.visibility = c : Za("#" + a, "visibility:" + c)
    }
  }, ab = function(a) {
    return null != /[\\\"<>\.;]/.exec(a) && typeof encodeURIComponent != r ? encodeURIComponent(a) : a
  }, r = "undefined", K = "object", sa = "application/x-shockwave-flash", Xa = "SWFObjectExprInst", E = window, j = document, O = navigator, bb = !1, na = [function() {
    if(bb) {
  ...
  }], N = [], ra = [], ca = [], ga, qa, Ja, Wa, T = !1, pa = !1, L, Ma, $a = !0, g, pb = typeof j.getElementById != r && typeof j.getElementsByTagName != r && typeof j.createElement != r, ta = O.userAgent.toLowerCase(), ua = O.platform.toLowerCase(), qb = ua ? /win/.test(ua) : /win/.test(ta), rb = ua ? /mac/.test(ua) : /mac/.test(ta), sb = /webkit/.test(ta) ? parseFloat(ta.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : !1, Na = !+"\v1", ha = [0, 0, 0], B = null;
  if(typeof O.plugins != r && typeof O.plugins["Shockwave Flash"] == K) {
    if((B = O.plugins["Shockwave Flash"].description) && !(typeof O.mimeTypes != r && O.mimeTypes[sa] && !O.mimeTypes[sa].enabledPlugin)) {
      bb = !0, Na = !1, B = B.replace(/^.*\s+(\S+\s+\S+$)/, "$1"), ha[0] = parseInt(B.replace(/^(.*)\..*$/, "$1"), 10), ha[1] = parseInt(B.replace(/^.*\.(.*)\s.*$/, "$1"), 10), ha[2] = /[a-zA-Z]/.test(B) ? parseInt(B.replace(/^.*[a-zA-Z]+(.*)$/, "$1"), 10) : 0
    }
  }else {
    if(typeof E.ActiveXObject != r) {
      try {
        var cb = new ActiveXObject("ShockwaveFlash.ShockwaveFlash");
        if(cb && (B = cb.GetVariable("$version"))) {
          Na = !0, B = B.split(" ")[1].split(","), ha = [parseInt(B[0], 10), parseInt(B[1], 10), parseInt(B[2], 10)]
        }
      }catch(Hb) {
      }
    }
  }
  g = {w3:pb, pv:ha, wk:sb, ie:Na, win:qb, mac:rb};
  g.w3 && ((typeof j.readyState != r && "complete" == j.readyState || typeof j.readyState == r && (j.getElementsByTagName("body")[0] || j.body)) && ba(), T || (typeof j.addEventListener != r && j.addEventListener("DOMContentLoaded", ba, !1), g.ie && g.win && (j.attachEvent("onreadystatechange", function() {
    ...
    T || (/loaded|complete/.test(j.readyState) ? ba() : setTimeout(arguments.callee, 0))
  }(), Va(ba)));
  ...
  S = {registerObject:function(a, b, c, f) {
  ...
  }, embedSWF:function(a, b, c, f, d, e, p, l, m, i) {
    var k = {success:!1, id:b};
    g.w3 && !(g.wk && 312 > g.wk) && a && b && c && f && d ? (U(b, !1), Ua(function() {
      c += "";
      f += "";
      var g = {};
      if(m && typeof m === K) {
      ...
      if(p && typeof p === K) {
        for(var q in p) {
          j.flashvars = typeof j.flashvars != r ? j.flashvars + ("&" + q + "=" + p[q]) : q + "=" + p[q]
        }
      }
      if(oa(d)) {
        n = La(g, j, b), g.id == b && U(b, !0), k.success = !0, k.ref = n
      }else {
        if(e && Ga()) {
          g.data = e;
          Ha(g, j, b, i);
    ...
    g.w3 && Za(a, b, c, f)
  }, addDomLoadEvent:Ua, addLoadEvent:Va, getQueryParamValue:function(a) {
    ...
    if(b) {
      ...
      for(var b = b.split("&"), c = 0;c < b.length;c++) {
        if(b[c].substring(0, b[c].indexOf("=")) == a) {
          return ab(b[c].substring(b[c].indexOf("=") + 1))
        }
      }
    }
    return""
    ...
    if(pa) {
      var a = I(Xa);
      a && ga && (a.parentNode.replaceChild(ga, a), qa && (U(qa, !0), g.ie && g.win && (ga.style.display = "block")), Ja && Ja(Wa));
      pa = !1
    }
  }};
  var va;
  ...
  i.__initialize = function() {
    if(!i.__initialized) {
      ...
      S.embedSWF("swf/WebSocketMainInsecure-0.9.swf", "webSocketFlash", "1", "1", "10.0.0", null, null, {hasPriority:!0, swliveconnect:!0, allowScriptAccess:"always"}, null, function(a) {
  ...
  z.nextTick = tb ? function(a) {
    setTimeout(a, 0)
  } : process.nextTick;
  var ub = {json:"application/json", jsonp:"application/javascript", xml:"application/xml", html:"text/html"};
  ...
  s.prototype.chooseTransport = function(a) {
    if(this.transport) {
      ...
      var b = this, c = this.transports.slice(), f = function(a) {
        var d = c.shift();
        d ? (e.logAction(e.LOG_MICRO, "ConnectionManager.chooseTransport()", "trying " + d), s.availableTransports[d].tryConnect(b, b.realtime.auth, b.options, function(c, l) {
          c ? f(a) : (e.logAction(e.LOG_MICRO, "ConnectionManager.chooseTransport()", "transport " + d + " connecting"), b.setupTransport(l), a(l))
        })) : a(null)
      };
      f(a)
    }
  ...
  var u = function(a, b, c) {
    ...
    t.call(this, a, b, c)
  }, Pa = "object" == typeof window, fb = Pa ? window.WebSocket || window.MozWebSocket : require("ws"), xb = Pa ? !1 : D, yb = "object" == typeof H ? H : require("../nodejs/lib/protocol/clientmessage_types"), xa = Pa ? d : require("thrift");
  n.inherits(u, t);
  ...
  u.prototype.connect = function() {
    ...
    this.auth.getAuthParams(function(c, f) {
      ...
      if(c) {
        ...
        try {
          var p = a.wsConnection = a.createWebSocket(b, f);
  ...
  v.prototype.connect = function() {
    ...
    this.auth.getAuthParams(function(c, f) {
      a.params = f;
      e.logAction(e.LOG_MINOR, "CometTransport.connect()", "authParams:" + v.paramStr(f));
      if(c) {
        a.abort(UIMessages.FAIL_REASON_REFUSED)
      }else {
        try {
  ...
  C.Request = function(a, b, c, d, e, g) {
    ...
    l.onreadystatechange = function() {
      if(4 == l.readyState) {
        clearTimeout(m);
        var a = null;
        l.status != i ? (a = Error("Unexpected response: statusCode = " + l.status), a.statusCode = l.status, a.statusText = l.statusText, g(a)) : (a = null, e ? l.response && (a = new D, a.buf = l.response, a.view = new DataView(a.buf)) : a = l.responseText, g(null, a))
      }
    };
    l.send(c)
  };
  ...
  G.Request = function(a, b, c, d, e) {
    var g = Ably._;
    this.callback = e;
    var i = this.requestId = zb++, l = this.timer = setTimeout(d ? 9E4 : 1E4, function() {
      j.abort()
    });
    ...
    Ably._["_" + i] = function(a) {
      clearTimeout(l);
      delete g["_" + i];
      j.aborted || (m.parentNode.removeChild(m), e(null, a))
    };
  ...
  P.isAvailable = function() {
    return Ab && S && 10 <= S.getFlashPlayerVersion().major && va
  };
  ...
  V.get = function(a, b, c, d) {
    function e() {
      a.auth.getAuthHeaders(function(g, i) {
        g ? d(g) : Http.get(a.baseUri + b, n.mixin(i, a.headers), c, function(b, c) {
    ...
    void 0 === d && "function" == typeof c ? (d = c, c = null) : d = hb;
    e()
  };
  V.post = function(a, b, c, d, e) {
    function g() {
      a.auth.getAuthHeaders(function(i, l) {
        i ? e(i) : Http.post(a.baseUri + b, n.mixin(l, a.headers), c, d, function(b, c) {
          b && 40140 == b.code ? a.auth.authorise({force:!0}, function(a) {
            a ? e(a) : g()
          }) : e(b, c)
        })
    ...
    void 0 === e && "function" == typeof d ? (e = d, d = null) : e = hb;
    g()
  };
  var Bb = function() {
  }, Cb = function(a) {
  ...
  da.prototype.requestToken = function(a, b) {
    ...
    var a = a || {}, b = b || Bb, a = n.mixin(n.copy(this.tokenOptions), a), c;
    if(a.authCallback) {
  ...
  ka.prototype.history = function(a, b) {
    V.get(this, "/events", a, b)
  };
  ka.prototype.stats = function(a, b) {
    V.get(this, "/stats", a, b)
  };
  ...
  n.inherits(W, w);
  W.prototype.presence = function(a, b) {
    e.logAction(e.LOG_MICRO, "Channel.presence()", "channel = " + this.name);
    V.get(this.rest, "/channels/" + this.name + "/presence", a, b)
  };
  W.prototype.history = function(a, b) {
    e.logAction(e.LOG_MICRO, "Channel.history()", "channel = " + this.name);
    V.get(this.rest, "/channels/" + this.name + "/history", a, b)
  };
  W.prototype.publish = function(a, b, c) {
    e.logAction(e.LOG_MICRO, "Channel.publish()", "channel = " + this.name + "; name = " + a);
    V.post(this.rest, "/channels/" + this.name + "/publish", {name:a, data:b}, c)
  };
  var Oa, q = function(a, b, c) {
    ...
    this.options = n.prototypicalClone(Eb, c);
  ...
  q.prototype.publish = function(a, b, c) {
    ...
    M.activeState(d) ? (d = new A.TMessage, d.name = a, d.payload = la.createPayload(b), "attached" == this.state ? (e.logAction(e.LOG_MICRO, "RealtimeChannel.publish()", "sending message"), a = new A.TChannelMessage, a.action = A.TAction.EVENT, a.channel = this.name, a.messages = [d], this.sendMessage(a, c)) : ("pending" != this.state && this.attach(), e.logAction(e.LOG_MICRO, "RealtimeChannel.publish()", "queueing message"), this.pendingEvents.push({message:d, listener:c}))) : c(d.defaultMessage)
  };
  q.prototype.onEvent = function(a) {
  ...
  q.prototype.attachImpl = function(a) {
  ...
  q.prototype.detachImpl = function(a) {
  ...
  q.prototype.subscribe = function() {
  ...
  q.prototype.subscribeAttached = function(a, b, c) {
  ...
  q.prototype.subscribeForEvent = function(a, b, c) {
  ...
  q.prototype.unsubscribeAttached = function(a, b, c) {
    if(null === a || a.__proto__ !== Array.prototype) {
  ...
  q.prototype.unsubscribeForEvent = function(a, b, c) {
  ...
  q.prototype.sendPresence = function(a, b) {
    var c = new A.TChannelMessage({action:A.TAction.PRESENCE, name:name});
    this.sendMessage(c, b)
  };
  q.prototype.onMessage = function(a) {
    switch(a.action) {
      case 5:
        this.setAttached(a);
        break;
      ...
      case 13:
        if(a = a.messages) {
          for(var b = Array(a.length), c = 0;c < b.length;c++) {
            var d = a[c];
            b[c] = new la(d.channelSerial, d.timestamp, d.name, la.getPayload(d.payload))
          }
          this.onEvent(b)
        }
        break;
  ...
  q.prototype.setAttached = function(a) {
    ...
    try {
      if(this.pendingEvents.length) {
        var b = new A.TChannelMessage({action:A.TAction.EVENT, channel:this.name, messages:[]}), c = new ia;
  ...
  q.prototype.setSubscribed = function(a) {
  ...
  q.prototype.setDetached = function(a) {
  ...
  q.prototype.setUnsubscribed = function(a) {
  ...
  q.prototype.setSuspended = function(a) {
  ...
  J.prototype.enterClient = function(a, b, c) {
    ...
    a.clientData = W.createPayload(b);
  ...
  J.prototype.setPresence = function(a, b) {
    ...
    for(var c = 0;c < a.length;c++) {
      ...
      "leave" == d.state ? delete this.clients[g] : h = this.clients[g] = la.getPayload(d.clientData);
  ...
  var la, Sa = function(a, b, c, d) {
    ...
    this.name = c;
    this.data = d
  }, x = "object" == typeof H ? H : require("../nodejs/lib/protocol/clientmessage_types"), Fb = {"[object Null]":function(a) {
    a.type = x.TType.NONE;
    return!0
