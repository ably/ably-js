...
export class Rest {
  ...
  async request(
    ...
    if (Utils.arrIn(Platform.Http.methodsWithBody, _method)) {
      return paginatedResource[_method as HttpMethods.Post](params, body) as Promise<HttpPaginatedResponse<unknown>>;
    } else {
  ...
  async batchPublish<T extends BatchPublishSpec | BatchPublishSpec[]>(
    ...
    const batchResults = (
      response._unpacked ? response._body : Utils.decodeBody(response._body, this.client._MsgPack, format)
