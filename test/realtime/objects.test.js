  ...
  describe('realtime/objects', function () {
    ...
    describe('Realtime with Objects plugin', () => {
      ...
      const writeApiScenarios = [
        ...
        {
          description: 'LiveCounter.create() throws on invalid input',
          action: async (ctx) => {
            const { root, realtimeObject } = ctx;

            await expectToThrowAsync(async () => LiveCounter.create(null), 'Counter value should be a valid number');
            await expectToThrowAsync(
              async () => LiveCounter.create(Number.NaN),
              'Counter value should be a valid number',
            );
            await expectToThrowAsync(
              async () => LiveCounter.create(Number.POSITIVE_INFINITY),
              'Counter value should be a valid number',
            );
            await expectToThrowAsync(
              async () => LiveCounter.create(Number.NEGATIVE_INFINITY),
              'Counter value should be a valid number',
            );
            await expectToThrowAsync(async () => LiveCounter.create('foo'), 'Counter value should be a valid number');
            await expectToThrowAsync(
              async () => LiveCounter.create(BigInt(1)),
              'Counter value should be a valid number',
            );
            await expectToThrowAsync(async () => LiveCounter.create(true), 'Counter value should be a valid number');
            await expectToThrowAsync(
              async () => LiveCounter.create(Symbol()),
              'Counter value should be a valid number',
            );
            await expectToThrowAsync(async () => LiveCounter.create({}), 'Counter value should be a valid number');
            await expectToThrowAsync(async () => LiveCounter.create([]), 'Counter value should be a valid number');
            await expectToThrowAsync(async () => LiveCounter.create(root), 'Counter value should be a valid number');
          },
        },
...

        {
          ...
          action: async (ctx) => {
            ...
            expect(map.size()).to.equal(2, 'Check map assigned to the object tree has the expected number of keys');
        ...
        {
          description: 'LiveMap.create() throws on invalid input',
          action: async () => {
            await expectToThrowAsync(async () => LiveMap.create(null), 'Map entries should be a key-value object');
            await expectToThrowAsync(async () => LiveMap.create('foo'), 'Map entries should be a key-value object');
            await expectToThrowAsync(async () => LiveMap.create(1), 'Map entries should be a key-value object');
            await expectToThrowAsync(async () => LiveMap.create(BigInt(1)), 'Map entries should be a key-value object');
            await expectToThrowAsync(async () => LiveMap.create(true), 'Map entries should be a key-value object');
            await expectToThrowAsync(async () => LiveMap.create(Symbol()), 'Map entries should be a key-value object');

            await expectToThrowAsync(
              async () => LiveMap.create({ key: undefined }),
              'Map value data type is unsupported',
            );
            await expectToThrowAsync(async () => LiveMap.create({ key: null }), 'Map value data type is unsupported');
            await expectToThrowAsync(
              async () => LiveMap.create({ key: BigInt(1) }),
              'Map value data type is unsupported',
            );
            await expectToThrowAsync(
              async () => LiveMap.create({ key: Symbol() }),
              'Map value data type is unsupported',
            );
          },
        },
      ...
      const pathObjectScenarios = [
...

        {
          ...
          action: async (ctx) => {
            ...
            await root.set('map', LiveMap.create({ key: 'map' }));
            await keyUpdatedPromise;
        ...
        {
          description: 'PathObject.value() returns undefined for non-existent paths',
          action: async (ctx) => {
            const { entryPathObject } = ctx;
        ...
        {
          description: 'PathObject collection methods throw errors for non-LiveMap objects',
          action: async (ctx) => {
            const { root, entryPathObject } = ctx;
            ...
            expect(() => primitivePathObj.size()).to.throw('Cannot get size of a non-LiveMap object');
            expect(() => [...primitivePathObj.entries()]).to.throw('Cannot iterate entries on a non-LiveMap object');
            expect(() => [...primitivePathObj.values()]).to.throw('Cannot iterate entries on a non-LiveMap object');
            expect(() => [...primitivePathObj.keys()]).to.throw('Cannot iterate entries on a non-LiveMap object');
          },
        },
        ...
        {
          description: 'PathObject map mutation methods throw errors for non-LiveMap objects',
          action: async (ctx) => {
            const { root, entryPathObject } = ctx;
            ...
            await root.set('counter', LiveCounter.create(5));
            await root.set('primitive', 'value');
            await keysUpdatedPromise;
        ...
        {
          description: 'PathObject counter methods throw errors for non-LiveCounter objects',
          action: async (ctx) => {
            const { root, entryPathObject } = ctx;
        ...
        {
          description: 'PathObject error handling for operations on non-existent paths',
          action: async (ctx) => {
            const { entryPathObject } = ctx;
            ...
            expect(nonExistentPathObj.value()).to.be.undefined;

            // These should throw due to path resolution failure
            expect(() => [...nonExistentPathObj.entries()]).to.throw('Could not resolve value at path');
            expect(() => [...nonExistentPathObj.keys()]).to.throw('Could not resolve value at path');
            expect(() => [...nonExistentPathObj.values()]).to.throw('Could not resolve value at path');
            expect(() => nonExistentPathObj.size()).to.throw('Could not resolve value at path');
            await expectToThrowAsync(
              async () => nonExistentPathObj.set('key', 'value'),
              'Could not resolve value at path',
            );
            await expectToThrowAsync(async () => nonExistentPathObj.remove('key'), 'Could not resolve value at path');
            await expectToThrowAsync(async () => nonExistentPathObj.increment(), 'Could not resolve value at path');
            await expectToThrowAsync(async () => nonExistentPathObj.decrement(), 'Could not resolve value at path');
          },
        },
        ...
        {
          description: 'PathObject.instance() returns undefined for primitive values and non-existent paths',
          action: async (ctx) => {
            const { root, helper, entryPathObject } = ctx;
            ...
            primitiveKeyData.forEach((keyData) => {
              const primitiveInstance = entryPathObject.get(keyData.key).instance();
              expect(primitiveInstance, 'Check instance is undefined for primitive path').to.be.undefined;
            });

            expect(entryPathObject.at('non.existing.path').instance(), 'Check instance is undefined for primitive path')
              .to.be.undefined;
          },
        },
      ...
      const instanceScenarios = [
        ...
        {
          description: 'DefaultInstance.value() returns undefined for LiveMap objects',
          action: async (ctx) => {
            const { root, entryPathObject } = ctx;
            ...
            expect(mapInstance.value(), 'Check DefaultInstance.value() for a LiveMap object returns undefined').to.be
              .undefined;
          },
        },
        ...
        {
          description: 'DefaultInstance collection methods throw errors for non-LiveMap objects',
          action: async (ctx) => {
            const { root, entryPathObject } = ctx;
            ...
            expect(() => primitiveInstance.size()).to.throw('Cannot get size of a non-LiveMap instance');
            expect(() => [...primitiveInstance.entries()]).to.throw('Cannot iterate entries on a non-LiveMap instance');
            expect(() => [...primitiveInstance.values()]).to.throw('Cannot iterate entries on a non-LiveMap instance');
            expect(() => [...primitiveInstance.keys()]).to.throw('Cannot iterate entries on a non-LiveMap instance');
          },
        },
        ...
        {
          description: 'DefaultInstance map mutation methods throw errors for non-LiveMap objects',
          action: async (ctx) => {
            const { root, entryPathObject } = ctx;
            ...
            await entryPathObject.set('counter', LiveCounter.create(5));
            await entryPathObject.set('primitive', 'value');
            await keysUpdatedPromise;
        ...
        {
          description: 'DefaultInstance counter methods throw errors for non-LiveCounter objects',
          action: async (ctx) => {
            const { root, entryPathObject } = ctx;
            ...
            const mapInstance = rootInstance.get('map');
            await expectToThrowAsync(
