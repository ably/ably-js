  ...
  describe('realtime/liveobjects', function () {
    ...
    describe('Realtime with LiveObjects plugin', () => {
      ...
      const applyOperationsDuringSyncScenarios = [
        ...
        {
          description: 'buffered object operation messages are discarded on non-resume ATTACHED message',
          action: async (ctx) => {
            ...
            // inject operation during first sync sequence, expect it to be discarded when non-resume ATTACHED arrives
            await objectsHelper.processObjectOperationMessageOnChannel({
              channel,
              serial: lexicoTimeserial('aaa', 0, 0),
              siteCode: 'aaa',
            ...
            // simulate non-resume ATTACHED message (RESUMED flag absent).
            // the realtime server caches the object state at the moment of channel attachment,
            // so buffered operations from before the attach must be discarded.
            ...
            // check root doesn't have data from operations received before non-resume ATTACHED
            expect(
              entryInstance.get('foo'),
              'Check buffered ops before non-resume ATTACHED were discarded and not applied on root',
            ).to.not.exist;

            // check root has data from operations received after non-resume ATTACHED
        ...
        {
          description: 'buffered object operation messages are NOT discarded on ATTACHED message with RESUMED flag',
          action: async (ctx) => {
            const { entryInstance, objectsHelper, channel, helper } = ctx;
            ...
            // inject operations, expect them to be kept when resume ATTACHED arrives
            await objectsHelper.processObjectOperationMessageOnChannel({
              channel,
              serial: lexicoTimeserial('aaa', 0, 0),
              siteCode: 'aaa',
            ...
            // simulate resume ATTACHED message (RESUMED flag set).
            // when resuming, the client continues from where it left off, so buffered operations
            // must be preserved as they were received after the original attachment point.
            await injectAttachedMessage(helper, channel, (1 << 7) | (1 << 2)); // HAS_OBJECTS flag and RESUMED flag

            // inject another operation after resume ATTACHED
            await objectsHelper.processObjectOperationMessageOnChannel({
              channel,
              serial: lexicoTimeserial('bbb', 0, 0),
              siteCode: 'bbb',
            ...
            // check root has data from both operations - before and after resume ATTACHED
            expect(entryInstance.get('foo').value()).to.equal(
              'bar',
              'Check root has data from operations received and buffered before resume ATTACHED',
            );
            expect(entryInstance.get('baz').value()).to.equal(
              'qux',
              'Check root has data from operations received after resume ATTACHED',
            );
          },
        },
