(function() {
  ...
  var ob = new D(0), $ = d.TTransport = function(a, b) {
    this.buf = a || ob;
    this.onFlush = b;
    this.reset()
  };
  ...
  if(typeof O.plugins != r && typeof O.plugins["Shockwave Flash"] == K) {
    if((B = O.plugins["Shockwave Flash"].description) && !(typeof O.mimeTypes != r && O.mimeTypes[sa] && !O.mimeTypes[sa].enabledPlugin)) {
      cb = !0, Na = !1, B = B.replace(/^.*\s+(\S+\s+\S+$)/, "$1"), ha[0] = parseInt(B.replace(/^(.*)\..*$/, "$1"), 10), ha[1] = parseInt(B.replace(/^.*\.(.*)\s.*$/, "$1"), 10), ha[2] = /[a-zA-Z]/.test(B) ? parseInt(B.replace(/^.*[a-zA-Z]+(.*)$/, "$1"), 10) : 0
    }
  }else {
  ...
  var u = function(a, b, c) {
    ...
    t.call(this, a, b, c)
  }, Pa = "object" == typeof window, gb = Pa ? window.WebSocket || window.MozWebSocket : require("ws"), xb = Pa ? !1 : D, yb = "object" == typeof H ? H : require("../nodejs/lib/protocol/clientmessage_types"), xa = Pa ? d : require("thrift");
  m.inherits(u, t);
  ...
  C.Request = function(a, b, c, d, e, g) {
    ...
    i.onreadystatechange = function() {
      if(4 == i.readyState) {
        clearTimeout(l);
        var a = null;
        i.status != n ? (a = Error("Unexpected response: statusCode = " + i.status), a.statusCode = i.status, a.statusText = i.statusText, g(a)) : (a = null, e ? i.response && (a = new D, a.buf = i.response, a.view = new DataView(a.buf)) : a = i.responseText, g(null, a))
      }
    };
    i.send(c)
  };
  ...
  p.prototype.publish = function(a, b, c) {
    ...
    M.activeState(d) ? (d = new A.TMessage, d.name = a, d.data = la.createPayload(b), "attached" == this.state ? (e.logAction(e.LOG_MICRO, "RealtimeChannel.publish()", "sending message"), a = new A.TChannelMessage, a.action = A.TAction.EVENT, a.channel = this.name, a.messages = [d], this.sendMessage(a, c)) : ("pending" != this.state && this.attach(), e.logAction(e.LOG_MICRO, "RealtimeChannel.publish()", "queueing message"), this.pendingEvents.push({message:d, listener:c}))) : c(d.defaultMessage)
  };
  ...
  var la, Sa = function(a, b, c, d) {
    ...
    this.name = c;
    this.data = d
  }, y = "object" == typeof H ? H : require("../nodejs/lib/protocol/clientmessage_types"), Fb = {"[object Null]":function(a) {
    a.type = y.TType.NONE;
    return!0
  ...
  Sa.createPayload = function(a) {
    var b = new y.TData, c = Gb[typeof a];
    if(c && c(b, a)) {
      return b
    }
