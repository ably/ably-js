...
define(['ably', 'shared_helper', 'async'], function(Ably, helper, async) {
	helper.describeWithCounter('realtime/channel', function (expect, counter) {
		var exports = {},
			_exports = {},
		...
		*/
		testOnAllTransports('channelinit0', function(realtimeOpts) { return function(done) {
			counter.expect(4);
			try {
		...
		* Base attach case
		*/
		testOnAllTransports('channelattach0', function(realtimeOpts) { return function(done) {
			counter.expect(1);
			try {
				var realtime = helper.AblyRealtime(realtimeOpts);
				realtime.connection.on('connected', function() {
					...
					channel0.attach(function(err) {
						if(err)
							...
							expect(true, 'Attach to channel 0 with no options');
							counter.assert();
						closeAndFinish(done, realtime);
					});
				});
		...
		*/
		testOnAllTransports('channelattach2', function(realtimeOpts) { return function(done) {
			counter.expect(1);
			try {
				var realtime = helper.AblyRealtime(realtimeOpts);
		...
		* Attach then detach
		*/
		testOnAllTransports('channelattach3', function(realtimeOpts) { return function(done) {
			counter.expect(1);
			try {
				var realtime = helper.AblyRealtime(realtimeOpts);
				realtime.connection.on('connected', function() {
					...
					channel0.attach(function(err) {
						...
						channel0.detach(function(err) {
							...
							if(channel0.state == 'detached')
								expect(true, 'Attach then detach to channel 0 with no options');
							else
								expect(false, 'Detach failed: State is '+channel0.state);
								counter.assert();
							closeAndFinish(done, realtime);
						});
					});
		...
		*/
		testOnAllTransports('channelattachempty', function(realtimeOpts) { return function(done) {
			counter.expect(1);
			try {
				var realtime = helper.AblyRealtime(realtimeOpts);
				realtime.connection.once('connected', function() {
					...
					channel0.attach(function(err) {
						if(err) {
							counter.expect(2);
							expect(true, 'Attach failed as expected');
							setTimeout(function() {
								expect(realtime.connection.state === 'connected', 'Client should still be connected');
								counter.assert();
								closeAndFinish(done, realtime);
							}, 1000);
							return;
						}
						expect(false, 'Unexpected attach success');
						closeAndFinish(done, realtime);
					});
				});
				monitorConnection(done, expect, realtime);
		...
		* and the connection to remain open
		*/
		testOnAllTransports('channelattachinvalid', function(realtimeOpts) { return function(done) {
			counter.expect(1);
			try {
				var realtime = helper.AblyRealtime(realtimeOpts);
				realtime.connection.once('connected', function() {
		...
		*/
		testOnAllTransports('publish_no_attach', function(realtimeOpts) { return function(done) {
			counter.expect(1);
			try {
				var realtime = helper.AblyRealtime(realtimeOpts);
				realtime.connection.once('connected', function() {
					realtime.channels.get('publish_no_attach').publish(function(err) {
						if(err) {
							expect(false, 'Unexpected attach failure: ' + helper.displayError(err));
							closeAndFinish(done, realtime);
							return;
						}
						expect(true, 'publish succeeded');
						counter.assert();
						closeAndFinish(done, realtime);
					});
				});
				...
				expect(false, 'publish_no_attach failed with exception: ' + e.stack);
				closeAndFinish(done, realtime);
			}
		}});
		...
		*/
		testOnAllTransports('channelattach_publish_invalid', function(realtimeOpts) { return function(done) {
			counter.expect(2);
			try {
				var realtime = helper.AblyRealtime(realtimeOpts);
				realtime.connection.once('connected', function() {
					realtime.channels.get(':hell').publish(function(err) {
						if(err) {
							expect(true, 'publish failed as expected');
				...
				expect(false, 'Channelattach_publish_invalid failed with exception: ' + e.stack);
				closeAndFinish(done, realtime);
			}
		}});
		...
		* and the connection to remain open
		*/
		testOnAllTransports('channelattach_invalid_twice', function(realtimeOpts) { return function(done) {
			counter.expect(1);
			try {
				var realtime = helper.AblyRealtime(realtimeOpts);
				realtime.connection.once('connected', function() {
					realtime.channels.get(':hell').attach(function(err) {
						if(err) {
							...
							realtime.channels.get(':hell').attach(function(err) {
								if(err) {
									counter.expect(3);
									expect(true, 'Attach (second attempt) failed as expected');
									setTimeout(function() {
										expect(realtime.connection.state === 'connected', 'Client should still be connected');
										counter.assert();
										closeAndFinish(done, realtime);
								...
								expect(false, 'Unexpected attach (second attempt) success');
								closeAndFinish(done, realtime);
							});
							return;
						}
		...
		it('channelattachOnceOrIfAfter', function(done) {
			...
			try {
				...
				channel.attach(function(err) {
					...
					expect(firedImmediately, 'whenState fired immediately as attached');
					counter.assert();
					closeAndFinish(done, realtime);
				});
				monitorConnection(done, expect, realtime);
		...
		*/
		it('channelattachOnceOrIfBefore', function(done) {
			...
			try {
				...
				expect(!firedImmediately, 'whenState should not fire immediately as not attached');
				monitorConnection(done, expect, realtime);
			} catch(e) {
				expect(false, 'Channel attach failed with exception: ' + e.stack);
				closeAndFinish(done, realtime);
		...
		testOnAllTransports('attachWithChannelParamsBasicChannelsGet', function(realtimeOpts) { return function(done) {
			counter.expect(3);
			...
			try {
					...
					channel.attach(function(err) {
						...
						testRealtime.connection.on('connected', function() {
							...
							async.series([
								...
								if(err)
									expect(false, 'Mode check failed with error: ' + displayError(err));
								testRealtime.close();
								counter.assert();
								closeAndFinish(done, realtime);
							});
				...
				expect(false, testName + ' failed with exception: ' + e.stack);
				closeAndFinish(done, realtime);
			}
		}});

		testOnAllTransports('attachWithChannelParamsBasicSetOptions', function(realtimeOpts) { return function(done) {
			counter.expect(3);
		...
		testOnAllTransports('subscribeAfterSetOptions', function(realtimeOpts) { return function(done) {
			counter.expect(1);
				...
				realtime.connection.on('connected', function() {
					...
					channel.attach(function(err) {
						...
						try {
							...
							expect(e.code).to.equal(40000, 'Check error code');
							expect(e.statusCode).to.equal(400, 'Check error status code');
							expect(e.message.includes('setOptions'), 'Check error message');
							counter.assert();
							closeAndFinish(done, realtime);
						}
					});
		...
		testOnAllTransports('setOptionsCallbackBehaviour', function(realtimeOpts) { return function(done) {
			counter.expect(6);
					...
					async.series([
						function(cb) {
							...
							channel.setOptions({
								...
								expect(!setOptionsReturned, 'setOptions failed to call back immediately, when no reattach is required');
								cb();
							});
							...
							channel.setOptions({
								...
								* channelUpdated listener or the setOptions listener first */
								helper.Utils.nextTick(function() {
									expect(setOptionsReturned, 'setOptions should return immediately and call back after the reattach');
									expect(channelUpdated, 'Check channel went to the server to update the channel params');
									cb();
								});
							});
							setOptionsReturned = true;
						...
						function(cb) {
							...
							channel.setOptions({
								...
								helper.Utils.nextTick(function() {
									expect(setOptionsReturned, 'setOptions should return immediately and call back after the reattach');
									expect(channelUpdated, 'Check channel went to the server to update the channel mode');
									cb();
								});
							});
							setOptionsReturned = true;
						...
						if(err)
							expect(false, testName + ' failed with error: ' + displayError(err));
		...
		testOnAllTransports('attachWithChannelParamsModesAndChannelModes', function(realtimeOpts) { return function(done) {
			counter.expect(3);
		...
		testOnAllTransports('attachWithChannelModes', function(realtimeOpts) { return function(done) {
			counter.expect(2);
		...
		testOnAllTransports('attachWithChannelParamsDeltaAndModes', function(realtimeOpts) { return function(done) {
			counter.expect(3);
					...
					async.series([
						...
						function(cb) {
							...
							channel.setOptions(channelOptions, function(err) {
								...
								expect(channel.modes == defaultChannelModes, 'Check channel options modes result');
								cb();
							});
						},
						function(cb) {
		...
		*/
		it('channelsubscribe0', function(done) {
			...
			try {
						...
						try {
							...
							setTimeout(function() {
								try {
									...
									expect(true, 'Subscribe then unsubscribe to channel6:event0 with no options');
									counter.assert();
									closeAndFinish(done, realtime);
								} catch(e) {
									expect(false, 'Unsubscribe failed with error: ' + e.stack);
									closeAndFinish(done, realtime);
								}
							}, 1000);
						} catch(e) {
							expect(false, 'Subscribe failed with error: ' + e);
							closeAndFinish(done, realtime);
						}
					});
				});
				...
				expect(false, 'Channel subscribe failed with exception: ' + e.stack);
				closeAndFinish(done, realtime);
			}
		});
			...
			try {
				...
				var unsubscribeTest = function() {
					...
					channelByEvent.publish('event', 'data', function(err) {
						expect(!err, 'Error publishing single event: ' + err);
						channelByListener.publish(null, 'data', function(err) {
							expect(!err, 'Error publishing any event: ' + err);
				...
				var listenerByEvent = function() {
					expect(true, 'received event "event" on channel');
					messagesReceived += 1;
					if (messagesReceived == 3) { unsubscribeTest(); }
				};
				var listenerNoEvent = function() {
					expect(true, 'received any event on channel');
					messagesReceived += 1;
					if (messagesReceived == 3) { unsubscribeTest(); }
				};
			...
			realtime.connection.once('connected', function() {
				channel.attach(function(err) {
					...
					channel.on('failed', function(stateChange) {
						expect(true, 'Channel was failed');
		...
		*/
		it('publish_no_queueing', function(done) {
			...
			channel.publish('foo', 'bar', function(err) {
				expect(err, 'Check publish while disconnected/connecting is rejected');
				counter.assert();
				closeAndFinish(done, realtime);
			});
		});
		...
		*/
		it('channel_attach_timeout', function(done) {
			...
			async.series([
				...
				function(cb) {
					channel.attach(function(err) {
						expect(err, 'Channel attach timed out as expected');
			...
			async.series([
				...
				function(cb) {
					...
					* until the connection reconnects */
					channel.sendMessage = function(msg) {
						expect(false, 'Channel tried to send a message ' + JSON.stringify(msg));
					};
					realtime.options.timeouts.realtimeRequestTimeout = 100;
				...
				function(cb) {
					channel.once(function(stateChange) {
						expect(stateChange.current).to.equal('suspended', 'Check that the channel goes back into suspended after attach fails');
