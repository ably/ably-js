...
define(['ably', 'shared_helper', 'async'], function(Ably, helper, async) {
	helper.describeWithCounter('realtime/resume', function (expect, counter) {
		var exports = {},
			_exports = {},
		...
		function resume_inactive(done, channelName, txOpts, rxOpts) {
				...
				function ph1TxOnce() {
					sendAndAwait('phase 1, message ' + rxCount, txChannel, rxChannel, function(err) {
						...
						if(++rxCount == count) {
							...
							expect(true, "phase 1: sent and received all messages")
							callback(null);
							return;
						}
				...
				function ph4TxOnce() {
					sendAndAwait('phase 4, message ' + rxCount, txChannel, rxChannel, function(err) {
						...
						if(++rxCount == count) {
							...
							expect(true, "phase 4: sent and received all messages")
							callback(null);
							return;
						}
			...
			phase0(function(err) {
				if(err) {
					expect(false, 'Phase 1 failed with err: ' + displayError(err));
					done();
					return;
				}
				phase1(function(err) {
					...
					phase2(function(err) {
						if(err) {
							expect(false, 'Phase 2 failed with err: ' + displayError(err));
							done();
							return;
						}
						phase3(function(err) {
							if(err) {
								expect(false, 'Phase 3 failed with err: ' + displayError(err));
								done();
								return;
							}
							phase4(function(err) {
								if(err) {
									expect(false, 'Phase 4 failed with err: ' + displayError(err));
									return;
								}
								counter.assert();
		...
		testOnAllTransports('resume_inactive', function(realtimeOpts) { return function(done) {
		...
		testOnAllTransports('resume_active', function(realtimeOpts) { return function(done) {
		...
		*/
		testOnAllTransports('resume_lost_continuity', function(realtimeOpts) { return function(done) {
			...
			async.series([
				...
				function(cb) {
					connection.once('connected', function(stateChange) {
						expect(stateChange.reason && stateChange.reason.code).to.equal(80008, 'Unable to recover connection correctly set in the stateChange');
						...
						expect(connection.connectionManager.connectionSerial).to.equal(-1, 'Check connectionSerial is reset by the new CONNECTED');
						expect(connection.connectionManager.connectionId !== 'ablyjs_tes', 'Check connectionId is set by the new CONNECTED');
						counter.assert();
						cb();
					});
				}
		...
		*/
		testOnAllTransports('resume_token_error', function(realtimeOpts) { return function(done) {
			...
			async.series([
				...
				function(cb) {
					...
					connection.once(function(stateChange) {
						expect(stateChange.current, 'disconnected', 'check connection disconnects first');
						cb();
					});
					connection.connectionManager.disconnectAllTransports();
				},
				function(cb) {
					connection.once('connected', function(stateChange) {
						expect(true, 'successfully reconnected after getting a new token');
						counter.assert();
						cb();
					});
				}
		...
		*/
		testOnAllTransports('resume_fatal_error', function(realtimeOpts) { return function(done) {
			var realtime = helper.AblyRealtime(realtimeOpts),
				connection = realtime.connection;
			...
			async.series([
				...
				function(cb) {
					connection.once('failed', function(stateChange) {
						expect(true, 'check connection failed');
		...
		*/
		it('channel_resumed_flag', function(done) {
			...
			async.series([
				...
				function(cb) {
					...
					realtimeTwo.connection.once('connected', function(stateChange) {
						if(stateChange.reason) {
							expect(false, 'Error while recovering: ' + JSON.stringify(stateChange.reason));
						}
						cb();
					});
				},
				...
				if(err) expect(false, helper.displayError(err));
				closeAndFinish(done, [realtime, realtimeTwo]);
			});
		});
			...
			try {
				...
				sender_channel.subscribe(function(message) {
					...
					receiver_channel.subscribe(function(message) {
						expect(JSON.stringify(testMessage) === JSON.stringify(message.data), 'Check rewound message.data');
						...
						resumed_receiver_channel.subscribe(function(message) {
							...
							expect(false, 'Rewound message arrived on attach resume');
