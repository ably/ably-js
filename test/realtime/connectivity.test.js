  ...
  describe('realtime/connectivity', function () {
    ...
    describe('configured_connectivity_check_url', function () {
      ...
      it('succeeds with scheme', function (done) {
        new helper.AblyRealtimePromise(options(urlScheme + successUrl)).http.checkConnectivity(function (err, res) {
          try {
            expect(res && !err, 'Connectivity check completed ' + (err && utils.inspectError(err))).to.be.ok;
          } catch (err) {
            done(err);
      ...
      it('fails with scheme', function (done) {
        new helper.AblyRealtimePromise(options(urlScheme + failUrl)).http.checkConnectivity(function (err, res) {
          try {
      ...
      it('succeeds with querystring', function (done) {
        new helper.AblyRealtimePromise(options(successUrl)).http.checkConnectivity(function (err, res) {
          try {
            expect(res && !err, 'Connectivity check completed ' + (err && utils.inspectError(err))).to.be.ok;
            done();
          } catch (err) {
      ...
      it('fails with querystring', function (done) {
        new helper.AblyRealtimePromise(options(failUrl)).http.checkConnectivity(function (err, res) {
          try {
            expect(!res, 'Connectivity check expected to return false').to.be.ok;
            done();
          } catch (err) {
      ...
      it('succeeds with plain url', function (done) {
        new helper.AblyRealtimePromise(options('sandbox-rest.ably.io/time')).http.checkConnectivity(function (
          err,
          res
        ) {
          try {
      ...
      it('fails with plain url', function (done) {
        new helper.AblyRealtimePromise(options('echo.ably.io')).http.checkConnectivity(function (err, res) {
          try {
            expect(!res, 'Connectivity check expected to return false').to.be.ok;
            done();
          } catch (err) {
    ...
    it('disable_connectivity_check', function (done) {
      new helper.AblyRealtimePromise({
