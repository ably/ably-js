  ...
  if (optMatch) {
    switch (optMatch[1]) {
      case 'algorithm':
        ...
        if (algorithm != 'aes') usage('Unrecognised algorithm: ' + algorithm);
        break;
      case 'mode':
        mode = optMatch[2];
        if (mode != 'cbc') usage('Unrecognised mode: ' + mode);
        break;
      case 'keylen':
      ...
      case 'encoding':
        encoding = optMatch[2];
        if (encoding != 'base64') usage('Unrecognised encoding: ' + encoding);
        break;
      case 'iv':
        try {
          var ivHex = optMatch[2].replace(/ /g, '');
