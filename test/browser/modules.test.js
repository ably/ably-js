  ...
  describe('browser/modules', function () {
    ...
    describe('Rest', () => {
      const restScenarios = [
        ...
        {
          description: 'call `auth.createTokenRequest()` with `queryTime` option',
          action: (client) => client.auth.createTokenRequest({ queryTime: true }),
        },
      ...
      describe('BaseRealtime without Rest', () => {
        ...
        it('allows `auth.createTokenRequest()` without `queryTime` option', async () => {
          const client = new BaseRealtime(ablyClientOptions(), { WebSocketTransport, FetchRequest });
