...
import { IConnectionManager, ITransportParams } from './connectionmanager';
...
const cometTransportClassFactory = (superclass: TransportClass, protocolMessageClass: IProtocolMessageConstructor) => {
  ...
  function protocolMessageFromRawError(err: ErrorInfo) {
    ...
    if (shouldBeErrorAction(err)) {
      return [protocolMessageClass.fromValues({ action: protocolMessageClass.Action.ERROR, error: err })];
    } else {
      return [protocolMessageClass.fromValues({ action: protocolMessageClass.Action.DISCONNECTED, error: err })];
    }
  }
  ...
  abstract class CometTransport extends superclass {
    ...
    pendingItems: null | Array<IProtocolMessage>;
    baseUri?: string;
    ...
    connect(): void {
      ...
      superclass.prototype.connect.call(this);
    ...
    onConnect(message: IProtocolMessage): void {
      ...
      superclass.prototype.onConnect.call(this, message);
    ...
    sendItems(items: Array<IProtocolMessage>): void {
      ...
      try {
        ...
        if (items && items.length)
          for (let i = 0; i < items.length; i++)
            this.onProtocolMessage(protocolMessageClass.fromDeserialized(items[i]));
      } catch (e) {
        Logger.logAction(
          Logger.LOG_ERROR,
    ...
    encodeRequest(requestItems: Array<IProtocolMessage>): string {
