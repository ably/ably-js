...
interface ClientIdList {
   ...
   * type, depending on context). */
  clientIds: string[];
  /** The total number of unidentified clients who have published an annotation with this name (or
   * type, depending on context), and so who are not included in the clientIds list. */
  totalUnidentified: number;
}

/** The per-name value for the multiple.v1 aggregation method. */
interface ClientIdCounts {
...
interface Total {
  /** The total number of clients who have published an annotation with this name (or
   * type, depending on context), identified or unidentified. */
  total: number;
}

/** The different possible values of the Message.summary map. */
type SummaryEntry = DistinctValues | UniqueValues | MultipleValues | ClientIdList | Total | unknown;
